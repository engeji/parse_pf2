...
.. sidebar:: Игровые соглашения
	...
	Например, правило гласит что при при атаке существа с состоянием :c_concealed:`скрыт`, вы должны совершить чистую проверку с КС 5 на попадание.
...
Например, если бы у вас был бонус состояния +1 от заклинания *героизм*, но штраф состояния -2 от :c_sickened:`тошноты`, вы бы применили их вместе к броску, так что *Героизм* все так же бы помог, хоть вы и чувствуете себя плохо.
...
Например, если способность дает вам состояние :c_frightened:`напуган` и после этого вы получили временный иммунитет к этой способности, то вы не теряете состояние :c_frightened:`напуган` из-за только что появившегося иммунитета, вы просто не становитесь снова напуганным, если до окончания временного иммунитета опять были целью этой способности.
...
.. sidebar:: Пример недуга
	...
	В описании предмета указано, что вы не можете уменьшить состояние :c_sickened:`тошнота`, пока находитесь под эффектом мышьяка, а так есть следующий текст о том, как работает недуг.
	...
	Из-за периода возникновения ничего не произойдет в течении 10 минут, но после них вы получить 1d4 урона ядом и получете состояние :c_sickened:`тошнота 1`.
	...
	Если вы достигли стадии 3 отравления, либо провалив стадию 2, либо критически провалив стадию 1, вы получите 2d6 урона ядом и состояние :c_sickened:`тошнота 3`".
...
Например, если недуг вызвал состояние :c_drained:`истощен`, но имеет макс.продолжительность 5 минут, то вы остаетесь истощенным даже после окончания недуга, как обычно для состояния :c_drained:`истощен`.
...
* Вы получаете состояние :c_dying:`при смерти 1`. Если эффект нокаутировавший вас был критически успешен со стороны атаковавшего или был результатом вашего критического провала, то вместо этого вы получаете :c_dying:`при смерти 2`. Если у вас есть состояние :c_wounded:`ранен` (см. :ref:`ch9--Recovery--Wounded`), то увеличьте значение :c_dying:`при смерти` на величину состояния :c_wounded:`ранен`. Если урон был от несмертельной атаки или эффекта, то вы не получаете состояние :c_dying:`при смерти`, а находитесь без сознания с 0 ОЗ.
...
Если вы получили урон когда уже находитесь при смерти, увеличьте ваше состояние :c_dying:`при смерти` на 1, или на 2, если урон был нанесен критическим попаданием или из-за вашего критического провала.
Если у вас есть состояние :c_wounded:`ранен`, не забудьте добавлять его величину к состоянию :c_dying:`при смерти`.
...
Когда вы при смерти, в начале каждого вашего хода вы должны сделать чистую проверку с КС равным 10 + ваше текущее значение :c_dying:`при смерти`, чтобы узнать становится вам лучше или хуже.
...
Если вы теряете состояние :c_dying:`при смерти` успешно пройдя проверку восстановления и все еще имеете 0 ОЗ, вы остаетесь без сознания, но можете очнуться как описано в этом состоянии (см. далее).
...
Если вам восстановили 1 ОЗ или более посредствам исцеления, вы теряете состояние :c_dying:`при смерти` и :c_unconscious:`без сознания` и можете действовать как обычно в свой следующий ход.
...
* Вы получили урон, он не опустил ОЗ до 0 (если урон снижает ОЗ до 0, то вы остаетесь без сознания и получаете состояние :c_dying:`при смерти` как обычно).
...
Каждый раз, когда вы теряете состояние :c_dying:`при смерти`, получаете состояние :c_wounded:`ранен 1` если у вас его не было.
...
Состояние :c_wounded:`ранен` пропадает если кто-то успешно восстановил вам ОЗ с помощью :ref:`skill--Medicine--Treat-Wounds` или вы восстановили полные ОЗ и отдохнули 10 минут.
...
Максимальное значение :c_dying:`при смерти`? при котором вы умрете уменьшается на значение :c_doomed:`обречен`.
Например, если вы были "обречены 1", вы умрете при достижении :c_dying:`при смерти 3` вместо обычных :c_dying:`при смерти 4`.
...
Если ваши ОЗ уменьшены до нуля эффектом смерти, вы мгновенно умираете без необходимости достигнуть :c_dying:`при смерти 4`.
...
Существо с регенерацией имеет дополнительные преимущества: пока его регенерация активна, его состояние :c_dying:`при смерти` никогда не увеличивается свыше :c_dying:`при смерти 3`.
...
Например, заклинание :ref:`spell--h--Haste` дает вам состояние :c_quickened:`ускорен`, но оно ограничивает на что вы можете потратить дополнительное действие.
...
Некоторые виды существ получают состояние :c_dazzled:`ослеплен` или :c_blinded:`слепота` от яркого света.
...
Существо или объект в темноте имеет состояние :c_hidden:`спрятан` или :c_undetected:`необнаружен`, если только смотрящий не обладает ночным зрением или точным чувством отличным от зрения (смотрите :ref:`ch9--Perception--Special-Senses`).
Существо без ночного зрения или другого способа ощущать в темноте имеет состояние :c_blinded:`слепота`, когда находится в темноте, хотя оно может видеть освещенные области за пределами темноты.
Если существо может видеть освещенную область, оно может нормально видеть существ в этой освещенной области.
На усмотрение Мастера, после нахождения в темноте, неожиданное появление яркого света может дать вам на короткое время состояние :c_dazzled:`ослеплен`.
...
Обычно, с помощью точного чувства, вы можете автоматически обнаружить существо, если только оно не имеет состояние :c_hidden:`спрятан` или загорожено окружением, в таком случае, вы можете использовать простое действие :ref:`action--Seek`, чтобы лучше обнаружить существо.
...
Обычно вы можете автоматически ощутить присутствие существа с помощью неточного чувства, но оно будет иметь состояние :c_hidden:`спрятан` вместо :c_observed:`замечен`.
...
Особые чувства дают бóльшую осведомленность, что позволяет существу с этими чувствами игнорировать или снижать эффекты состояний :c_undetected:`необнаружен`, :c_hidden:`спрятан` и :c_concealed:`скрыт` (подробнее описано далее в разделе :ref:`ch9--Detecting-Creatures`), когда дело касается недостатков обычного зрения.
...
.. sidebar:: Обнаружение другими чувствами (Detecting With Other Senses)
	...
	Это значит, что его добыча будет иметь состояние :c_concealed:`скрыт` в шумном помещении, :c_hidden:`спрятан` при сильном грохоте, или даже "невидимым" в области действия заклинания :ref:`spell--s--Silence`.
...
За исключением состояния "невидимость", эти состояния относительны наблюдателя, и возможно такое, что для вас существо имеет состояние :c_observed:`замечен`, но для ваших союзников оно "спрятано".
...
Если вы не можете заметить существо, то оно имеет состояние :c_hidden:`спрятан`, :c_undetected:`необнаружен` или :c_unnoticed:`незамечен`, и вам надо учитывать эти ограничения если вы собираетесь целиться в него.
Даже если существо имеет состояние :c_observed:`замечен`, оно так же может иметь состояние :c_concealed:`скрыт`.
...
Существо с состоянием :c_hidden:`спрятан` едва заметно.
Вы знаете какое пространство занимает существо с состоянием :c_hidden:`спрятан`, но не более.
...
Независимо от деталей, для существа с состоянием :c_hidden:`спрятан` вы являетесь "застигнутыми врасплох".
...
Если существо имеет состояние :c_undetected:`необнаружен`, то вы не знаете какое пространство оно занимает, вы для него считаетесь застигнутыми врасплох, и вы не можете с легкостью целиться в него.
...
Если существо имеет состояние :c_undetected:`необнаружен`, это не обязательно значит, что вы не осведомлены о его присутствии, вы можете подозревать о необнаруженных существах в помещении с вами, хотя и не можете найти в каком пространстве они находятся.
Состояние :c_unnoticed:`незамечен` скрывает существ о которых вы абсолютно не подозреваете.
...
Состояния :c_concealed:`скрыт` и :c_invisible:`невидимый` описывают обстоятельства, при которых существо трудно заметить.
...
Когда вы целитесь в существо, которое для вас имеет состояние :c_concealed:`скрыт`, то должны сделать чистую проверку с КС 5 прежде, чем сделаете бросок для определения вашего эффекта.
...
Существо в легком тумане имеет состояние :c_observed:`замечен`, хотя и имеет состояние :c_concealed:`скрыт`.


Невидимость (Invisible)
""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""

Существо с состоянием "невидимость" (от заклинания :ref:`spell--i--Invisibility` или от :ref:`item--Invisibility-Potion`) автоматически имеет состояние :c_undetected:`необнаружен` для любых существ полагающихся только на зрение, как на их единственное точное чувство.
...
Если существо для вас "замечено" когда оно становится невидимым, то оно начинает с состоянием :c_hidden:`спрятан`, так как вы знаете где оно было когда стало невидимым, хотя оно может использовать :ref:`skill--Stealth--Sneak`, чтобы стать "необнаруженным".
...
* **Потратить все Очки Геройства** (минимум 1) чтобы избежать смерти. Вы можете сделать это, когда состояние :c_dying:`при смерти` увеличится. Вы теряете состояние :c_dying:`при смерти` и стабилизируетесь с 0 ОЗ. Вы не получаете состояние :c_wounded:`ранен` или увеличиваете его значение от потери состояния :c_dying:`при смерти`, но если у вас уже было состояние :c_wounded:`ранен`, вы не лишаетесь его и не уменьшаете его.
...
* Если у вас есть состояние с продолжительным уроном, вы получаете урон в этот момент. После того как вы получите урон, вы можете сделать чистую проверку, чтобы закончить продолжительный урон. Потом вы пытаетесь пройти спасброски против действующих недугов. Многие другие состояния меняются в конце вашего хода, как например :c_frightened:`напуган`, чье значение уменьшается. Это действия происходят в последнюю очередь, когда вы получили продолжительный урон и сделали чистые проверки от него, и сделали спасброски от любых недугов.
...
Это длится пока вы не покините свое текущее пространство, используете атакующее действие, потеряете сознание (состояние :c_unconscious:`без сознания`), или завершите этот эффект используя свободное действие.
...
Вы пытаетесь вырваться, когда имеете состояние :c_grabbed:`схвачен`, :c_immobilized:`обездвижен` или :c_restrained:`сдерживаем`.
...
Мастер совершает за вас одиночную тайную проверку Восприятия и сравнивает результат с КС Скрытности любых существ с состоянием :c_undetected:`необнаружен` или :c_hidden:`спрятан` в области, или с КС обнаружения каждого объекта в области (по решению Мастера или кого-то, кто использовал :ref:`skill--Stealth--Conceal-an-Object`).
...
| **Критический успех**: Если вы искали существ, то любое существо с состоянием :c_undetected:`необнаружен` или :c_hidden:`спрятан` становится для вас "замеченным". Если вы искали объект, то узнаете его местоположение.
| **Успех**: Если вы искали существ, то любое существо с состоянием :c_undetected:`необнаружен` против которого вы прошли проверку, становится для вас "спрятанным" а не "необнаруженным"; любое существо с состоянием :c_hidden:`спрятан`, становится для вас "замеченным". Если вы искали объект, то по решению Мастера вы узнаете его местоположение или получаете подсказку о его примерном местонахождении.
...
**Требования**: Существо имеет состояние :c_undetected:`необнаружен` для одного или более союзника, но не "необнаружено" для вас
...
Существо получает состояние :c_flat_footed:`застигнут врасплох` (получая штраф обстоятельства -2 КБ) против тех существ, которые взяли его в тиски.
