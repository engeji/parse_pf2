...
Вы можете бросить проверку Скрытности против врага, к которому подкрадываетесь, как если бы были "спрятаны", даже если вы :c_observed:`замечены` им.
...
Мастер делает за вас тайную проверку Восприятия против КС Обмана или Скрытности (в зависимости от того, что больше) выбранного вами врага, который вовлечен в бой и который не имеет для вас состояния :c_concealed:`скрыт`, :c_hidden:`спрятан` или :c_undetected:`необнаружен`.
...
Любое существо, которое имеет состояние :c_frightened:`напуган`, становится застигнуто врасплох для ваших атак.
...
До конца вашего хода, для следующего :ref:`Удара (Strike) <action--Strike>` метательным оружием, вы получаете бонус обстоятельства +1 к своему броску атаки и игнорируете состояние цели :c_concealed:`скрыт`, а также небольшое и стандартное укрытие цели;
...
| **Критический успех**: Цель невредима
| **Успех**: Цель получает состояние :c_slowed:`замедлен 1` до конца вашего следующего хода
| **Провал**: Цель получает состояние :c_slowed:`замедлен 2` до конца вашего следующего хода
| **Критический провал**: Цель получает состояние :c_paralyzed:`парализован` до конца вашего следующего хода
...
Когда вы критически попадаете атакой ближнего боя и наносите урон застигнутому врасплох противнику, то цель провоцирует реакцию :ref:`class-feature--Fighter--Attack-Of-Opportunity` от одного союзника по вашему выбору, у которого она есть, как если бы враг использовал действие с признаком :t_manipulate:`воздействие`.
...
**Триггер**: Враг, лежащий ничком в пределах вашей досягаемости :ref:`Встает (Stands) <action--Stand>`
...
Вы используете :ref:`class-feat--rogue--Perfect-Distraction` даже, если были с состоянием :c_observed:`замечен`, при условии, что заканчиваете свое передвижение когда :ref:`Крадетесь (Sneak) <skill--Stealth--Sneak>` имея состояние :c_concealed:`скрыт` или в месте с укрытием или большим укрытием.
