...
В конце вашего хода, когда вы уменьшаете свое состояние "напуган" на 1, вы еще снижаете его на 1 для всех союзников в пределах 15 футов от вас.
...
Если ваше следующее действие - использование :ref:`spell--focus--Lay-On-Hands`, помимо других преимуществ, вы можете попытаться противодействовать эффекту страха или эффекту наложившему на цель состояние "парализован".
...
Дополнительно, враг, который заканчивает свой ход в пределах 15 футов от вас, не может снизить значение своего состояния "напуган" ниже 1.
...
Если ваше следующее действие - сотворять :ref:`spell--focus--Touch-of-Corruption` на живое существо, цель еще получает состояние "ослаблен 1" на 1 минуту, если оно проваливает свой спасбросок ("ослаблен 2" при крит.провале).
...
Когда вы используете :ref:`class-feat--Champion--Mercy`, вместо этого можете совершить проверку противодействия, чтобы снять состояния "неуклюжесть" или "ослаблен", используя источник этого состояния для определения уровня противодействия и его КС.
...
Когда вы используете :ref:`class-feat--Champion--Mercy`, то вместо этого можете попытаться противодействовать состояниям "слепота", "глухота", "тошнота" или "замедлен".
...
Когда вы используете :ref:`class-feat--Champion--Cruelty`, выберите хотите ли вы, как обычно, дать существу состояние "ослаблен", дать ему состояние "неуклюжесть 1" при провале спасброска ("неуклюжесть 2" при крит.провале), или дать состояние "одурманен 1" при провале спасброска ("одурманен 2" при крит.провале).
...
Когда вы используете :ref:`class-feat--Champion--Mercy`, то вместо этого можете сделать проверку противодействия, чтобы снять состояния "замешательство" или "одурманен", используя источник этого состояния для определения уровня противодействия и его КС.
Если состояние было вызвано продолжающимся эффектом и вы не убрали этот эффект, то состояние возвращается в конце вашего следующего хода.
...
Если у вас есть :ref:`class-feat--Champion--Weight-of-Guilt`, то после "одурманен 2" от :ref:`class-feature--Champion--Reaction--Glimpse-of-Redemption`, враг "одурманен 1" на 1 минуту или пока он не потеряет заклинание из-за провала чистой проверки одурманивания, в зависимости от того, что произойдет раньше.
...
Если вы попадаете, цель получает удвоенный дополнительный урон даваемый вашей реакцией чемпиона и должно успешно совершить спасбросок Стойкости с вашим КС класса, иначе получит состояние "истощен 1".
...
Когда вы крит.попадаете с помощью :ref:`class-feat--Champion--Blade-of-Justice` или :ref:`class-feature--Champion--Reaction--Retributive-Strike`, вашей атаке добавляется 1 дополнительная кость урона, и цель получает состояние "замедлен 1" на свой следующий ход.
