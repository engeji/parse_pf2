.. include:: /helpers/roles.rst

.. rst-class:: archetype-multiclass
.. _archetype--Wizard:

Волшебник (`Wizard (Archetype) <https://2e.aonprd.com/Archetypes.aspx?ID=12>`_)
----------------------------------------------------------------------------------------------------------

Вы погрузились в арканные искусства и благодаря дисциплине и академическому обучению, научились колдовать несколько заклинаний.


Мультиклассовые персонажи волшебники (Multiclass Wizard Characters)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Архетип волшебника дает гибкость подготавливаемых заклинаний, идеальную для персонажей ищущих дополнительную выгоду.


.. _arch-feat--Wizard--Dedication:

Посвящение волшебника (`Wizard Dedication <https://2e.aonprd.com/Feats.aspx?ID=742>`_) / 2
""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""

- архетип
- посвящение
- мультикласс

**Предварительные условия**: Интеллект 14

----------

Вы сотворяете заклинания как волшебник, получая книгу заклинаний с 4 обычными арканным чарами на ваш выбор.
Вы получаете активность :ref:`action--Cast-a-Spell`.
Вы можете каждый день подготовить 2 чар из своей книги заклинаний.
Вы обучены атакам заклинаниями и КС арканных заклинаний.
Ваша ключевая характеристика колдовства для заклинаний архетипа волшебника - Интеллект, а заклинания - арканные заклинания волшебника.
Вы становитесь обучены Аркане; если вы уже были обучены Аркане, то становитесь обучены другому навыку на свой выбор.
Выберите 1 арканную школу магии; вы не получаете никаких возможностей от своего выбора школы.

**Особенность**: Вы не можете выбрать другую способность посвящения, пока не получите 2 другие способности из архетипа волшебника.

.. versionadded:: /errata-CR-r3
	Теперь вместе с посвящением надо выбрать арканную школу.
	До этого, чтобы взять некоторые способности волшебника, необходимо было предварительно взять :ref:`arch-feat--Wizard--Arcane-School-Spell`, в отличие от чародея, барда и друида, которые вместе с посвящением выбирали наследие крови, музу или орден соответственно.


.. _arch-feat--Wizard--Arcane-School-Spell:

Заклинание арканной школы (`Arcane School Spell <https://2e.aonprd.com/Feats.aspx?ID=743>`_) / 4
"""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""

- архетип

**Предварительные условия**: :ref:`arch-feat--Wizard--Dedication`

----------

Вы получаете начальное заклинание школы.
Если у вас еще не было запаса Очков Фокусировки, то вы его получаете с 1 ОФ, которое можете восстановить используя :ref:`action--Refocus` для обучения (больше подробностей о арканных школах смотрите в описании класса волшебника, в разделе :ref:`class-feature--Wizard--Arcane-School`).

.. versionchanged:: /errata-CR-r3
	Выбор арканной школы перенесен в посвящение.


.. _arch-feat--Wizard--Basic-Arcana:

Основная аркана (`Basic Arcana <https://2e.aonprd.com/Feats.aspx?ID=744>`_) / 4
""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""

- архетип

**Предварительные условия**: :ref:`arch-feat--Wizard--Dedication`

----------

Вы получаете способность волшебника 1-го или 2-го уровня.


.. _arch-feat--Wizard--Basic-Spellcasting:

Основное колдовство волшебника (`Basic Wizard Spellcasting <https://2e.aonprd.com/Feats.aspx?ID=745>`_) / 4
"""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""

- архетип

**Предварительные условия**: :ref:`arch-feat--Wizard--Dedication`

----------

Вы получаете преимущества :ref:`arch--Basic-Spellcasting`.
Каждый раз, когда вы получаете слот заклинания нового уровня от архетипа волшебника, добавьте 2 обычных заклинания этого уровня в свою книгу заклинаний.


.. _arch-feat--Wizard--Advanced-Arcana:

Продвинутая аркана (`Advanced Arcana <https://2e.aonprd.com/Feats.aspx?ID=746>`_) / 6
""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""

- архетип

**Предварительные условия**: :ref:`arch-feat--Wizard--Basic-Arcana`

----------

Вы получаете 1 способность волшебника.
С целью соответствия предварительным условиям, ваш уровень волшебника равен половине уровня персонажа.

**Особенность**: Вы можете выбрать эту способность более одного раза.
Каждый раз, выбирая ее, вы получаете другую способность волшебника.


.. _arch-feat--Wizard--Arcane-Breadth:

Арканная широта (`Arcane Breadth <https://2e.aonprd.com/Feats.aspx?ID=747>`_) / 8
""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""

- архетип

**Предварительные условия**: :ref:`arch-feat--Wizard--Basic-Spellcasting`

----------

Вы можете колдовать больше заклинаний каждый день.
Увеличьте количество слотов заклинаний, получаемых от способностей архетипа волшебника на 1 для каждого уровня заклинаний, кроме двух ваших наибольших слотов заклинаний волшебника.


.. _arch-feat--Wizard--Expert-Spellcasting:

Экспертное колдовство волшебника (`Expert Wizard Spellcasting <https://2e.aonprd.com/Feats.aspx?ID=748>`_) / 12
"""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""

- архетип

**Предварительные условия**: мастер Арканы, :ref:`arch-feat--Wizard--Basic-Spellcasting`

----------

Вы получаете преимущества :ref:`arch--Expert-Spellcasting`.


.. _arch-feat--Wizard--Master-Spellcasting:

Мастерское колдовство волшебника (`Master Wizard Spellcasting (Archetype) <https://2e.aonprd.com/Feats.aspx?ID=749>`_) / 18
"""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""

- архетип

**Предварительные условия**: легенда Арканы, :ref:`arch-feat--Wizard--Expert-Spellcasting`

----------

Вы получаете преимущества :ref:`arch--Master-Spellcasting`.





.. include:: /helpers/actions.rst