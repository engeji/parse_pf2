...
Вот некоторые примерные правила дуэли.
...
В отличие от других подсистем, описанных в этой главе, боевая дуэль действует почти так же, как и обычный бой, за некоторыми исключениями.
...
Ни один из дуэлянтов не знает, какой тип броска инициативы был использован другим - неожиданность и использование дуэльных действий являются частью дуэльной стратегии.
...
**Триггер**: Вы совершаете :ref:`action--Strike` блаженного боя по противнику, но еще не совершили бросок
...
Если, когда было использовано это умение, ваш оппонент использовал для инициативы Восприятие, то вместо этого он становится :c_frightened:`напуган 2`.
...
Если, когда было использовано это умение, ваш оппонент использовал для инициативы Запугивание, то он еще получает для второго броска атаки штраф обстоятельства -2.
...
Ваш противник :c_flat_footed:`застигнут врасплох` против атаки.
Если, когда было использовано это умение, ваш оппонент использовал для инициативы Обман, то вместо этого он :c_flat_footed:`застигнут врасплох` до начала вашего следующего хода.
...
Если они обучены этому навыку, то получают фокус магического обычая этого навыка, что позволяет им сосредоточиться на определенной магической школе, соответствующей проверке, которую они сделали для своего броска инициативы.
...
Они по-прежнему получают реакцию :ref:`action--GMG--Dueling-Counter` и действие :ref:`action--GMG--Change-Tradition-Focus`, хотя Change Tradition Focus менее полезно в боевых дуэлях.
...
**Триггер**: Ваш оппонент :ref:`Сотворяет заклинание (Casts a Spell) <action--Cast-a-Spell>` того же обычая, что и ваш фокус обычая

**Требования**: Вы в дуэле, и обладаете фокусом обычая
