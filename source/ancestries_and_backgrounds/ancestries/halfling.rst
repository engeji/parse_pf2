...
Полурослик (`Halfling <https://2e.aonprd.com/Ancestries.aspx?ID=5>`_)
=============================================================================================================
...
.. sidebar:: hidden
	...
	Выберите из Дварфийский, Эльфийский, Гномий, Гоблинский, и тех к которым у вас есть доступ (распространенные в вашем регионе).


	.. rubric:: Признаки

	:t_halfling:`Полурослик`

	:t_humanoid:`Гуманоид`


	.. _ancestry_feature--Halfling--Keen-Eyes:
	.. rubric:: Зоркие глаза (Keen Eyes)

	Ваши глаза остры, что позволяет различать мелкие детали в скрытых или даже невидимых существах, которые другие могут пропустить.
...
Полурослики почитают богов, которые либо даруют удачу, как :doc:`/lost_omens/Deity/Core/Desna`, либо поощряют коварство, как :doc:`/lost_omens/Deity/Core/Norgorber`, и многие ценят роль :doc:`Кайдэна Кайлина </lost_omens/Deity/Core/Cayden-Cailean>` как освободителя, а также любые религии, распространенные среди других народов окружающих их.
...
Когда при броске спасброска вы получаете успех против :t_emotion:`эмоционального` эффекта, то вместо этого вы получаете крит.успех.
...
Вы получаете два дополнительных языка на свой выбор, выбираемых из доступных вам :ref:`распространенных <table--2-1>` и :ref:`необычных <table--2-2>`, а всякий раз, когда вы берете способность :ref:`feat--Multilingual`, то получаете другой новый язык.
...
| **Провал**: Цель получает состояние :c_clumsy:`неуклюжесть 1` на 1 минуту
| **Критический провал**: Цель получает состояние :c_clumsy:`неуклюжесть 2` на 1 минуту
...
С целью определения вашего уровня мастерства, воинское оружие полуросликов считается простым оружием, а продвинутое оружие полуросликов считается воинским оружием.
...
Когда Балансируете или Карабкаетесь, вы не :c_flat_footed:`застигнуты врасплох`.
...
Когда вы попадаете атакой из пращи по существу большого размера или больше, увеличьте размер кости оружия на один шаг (см. :ref:`Увеличение размера кости (Increasing Die Size) <ch6--Damage--Increasing-Die-Size>`).
...
Когда при броске вы получаете успех проверки :ref:`action--Escape` или спасброска против эффекта, который наложит на вас состояние :c_grabbed:`схвачен` или :c_restrained:`сдерживаем`, вместо этого вы получаете крит.успех.
Всякий раз, когда существо при броске проверки :ref:`skill--Athletics--Grapple` получает провал, то оно получает крит.провал.
Если существо использует на вас умение :ref:`cr_ability--Grab`, то оно должно совершить успешную проверку Атлетики, чтобы схватить вас, вместо автоматического срабатывания.
...
Вы получаете бонус обстоятельства +2 к проверкам Восприятия когда используете простое действие :ref:`action--Sense-Motive`, чтобы заметить :t_enchantment:`очарованного` или :t_possession:`одержимого` персонажа.
Если вы сами не используете :ref:`action--Sense-Motive` на :t_enchantment:`очарованном` или :t_possession:`одержимом` персонаже, Мастер кидает за вас :t_secret:`тайную` проверку без обычного бонуса обстоятельства и со штрафом обстоятельства -2, чтобы как-то потенциально заметить очарование или одержимость.

В дополнение к использованию с проверками навыка, вы можете использовать простое действие :ref:`action--Aid`, чтобы дать бонус спасброску другого существа или другой проверки, чтобы преодолеть :t_enchantment:`очарование` или :t_possession:`одержимость`.
Как обычно для :ref:`action--Aid`, вам надо подготовиться используя действие в ваш ход, чтобы поддержать существо в борьбе против этого эффекта.
...
Вы с детства возитесь с узлами, замками и кузнечными головоломками, чтобы хоть чем-то занять свои руки.
...
Если вы проваливаете проверку Обмана, чтобы :ref:`skill--Deception--Create-a-Diversion`, то гуманоидные существа не знают что вы пытались отвлечь их, если только при броске вы не получаете крит.провал.
...
Чтобы понять ваше сообщение, подслушивающие тоже могут попытаться сделать проверку Восприятия с вашим КС Обмана.
...
Когда вы в толпе или переполненной области города, вы можете попытаться использовать :ref:`skill--Stealth--Hide` и :ref:`skill--Stealth--Sneak`, даже когда :c_observed:`замечены`.
При успехе, вы не :c_hidden:`спрятаны` или :c_undetected:`необнаружены`, но другие существа просто не обращают на вас особого внимания, даже если они могут видеть вас.
...
Если вы используете :ref:`ancestry-feat--Halfling--Luck` при провале проверки навыка, которому вы нетренированы, то при перебрасывании спровоцировавшей проверки навыка, вы можете добавить бонус мастерства равный вашему уровню, вместо 0.
...
**Триггер**: Враг большого размера или больше закончил рядом с вами свое действие :t_move:`движения`
...
Когда при броске спасброска вы получаете успех против :t_emotion:`эмоционального` эффекта, то место этого вы получаете крит.успех.
Если ваше наследие - :ref:`ancestry-heritage--Halfling--Gutsy`, когда при броске спасброска против :t_emotion:`эмоционального` эффекта вы получаете крит.провал, то вместо этого получаете простой провал.
...
.. versionchanged:: /errata-LOCG-2
	Удалено последнее предложение про способность "Легендарный скалолаз (Legendary Climber)" из плэй-теста, объединенная на релизе вместе с :ref:`feat--Quick-Climb`.
...
Когда вы помогаете другу с задачей, то находите много способов помочь и избежать пагубного вмешательства.
...
Например, вы можете занять место с :t_giant:`гигантом` или :t_dragon:`драконом`, но не со :t_ooze:`слизью`.
...
Вы можете раз в день сотворять на себя :ref:`spell--f--Freedom-of-Movement` как врожденное природное заклинание.
...
Вам больше не нужно укрытие или быть :c_concealed:`скрытым`, чтобы :ref:`skill--Stealth--Hide` или :ref:`skill--Stealth--Sneak`.
...
Находясь на открытом воздухе в городской среде, вы можете :ref:`Шагать (Step) <action--Step>` на сложной местности, а враги на сложной местности для вас :c_flat_footed:`застигнуты врасплох`.
...
Вы можете быть в том же пространстве, что и лежащее :c_prone:`ничком` существо большого размера и более, даже если это не ваш союзник.
...
**Триггер**: Вы успешно используете Скрытность, чтобы :ref:`skill--Stealth--Hide` и стать :c_hidden:`спрятанным` от всех своих текущих врагов, или использовали Скрытность, чтобы :ref:`skill--Stealth--Sneak` и стали :c_undetected:`необнаружены` для всех своих текущих врагов
...
Вы становитесь :c_invisible:`невидимы` на 1 минуту или пока не совершите враждебное действие, в зависимости от того, что произойдет раньше.
Выберите место в пределах 10 футов от вас.
Пока не закончится ваша невидимость, для всех, кто пытается вас найти, кажется, что вы :c_hidden:`спрятаны` в этом месте.
Если ищущий имеет ясное доказательство того, что вы не там, он больше не думает, что вы спрятаны там, но и не узнает вашего актуального местоположения.
