.. include:: /helpers/roles.rst

.. rst-class:: gear-description

.. _Weapon-description:

Описание оружия (Weapon Descriptions)
--------------------------------------------------------------------------------------------------------

Здесь описано все оружие перечисленное в :ref:`table--6-7` и последующих.

.. _weapon--Club:

Дубинка (`Club <https://2e.aonprd.com/Weapons.aspx?ID=2>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Это кусок крепкого дерева в сформирован или приспособлен так, чтобы бить врага.
Дубинки могут быть замысловато вырезанными предметами боевого искусства или такими же простыми, как ветка или кусок дерева.

.. _weapon--Dagger:

Кинжал (`Dagger <https://2e.aonprd.com/Weapons.aspx?ID=3>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Это маленькое острое оружие держится одной рукой и используется, чтобы бить существ в ближнем бою.
Еще его можно метать.

.. _weapon--Light-Mace:

Легкая булава (`Light Mace <https://2e.aonprd.com/Weapons.aspx?ID=5>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Легкая булава имеет короткую деревянную или металлическую рукоятку оканчивающуюся компактным металлическим навершием.
Используемая как дубинка, она наносит тяжелые дробящие удары, но с дополнительной силой от металлических выступов или шипов навершия.

.. _weapon--Mace:

Булава (`Mace <https://2e.aonprd.com/Weapons.aspx?ID=7>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

С крепкой рукоятью и тяжелым металлическим навершием, булава прочна и позволяет ее владельцу наносить мощные удары и пробивать доспехи.

.. _weapon--Morningstar:

Моргенштерн (`Morningstar <https://2e.aonprd.com/Weapons.aspx?ID=8>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Это оружие имеет короткую рукоять, оканчивающуюся металлическим шаром с шипами.

.. _weapon--Sickle:

Серп (`Sickle <https://2e.aonprd.com/Weapons.aspx?ID=9>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Изначально это был сельскохозяйственный инструмент, используемый для жатвы зерна, это одноручное оружие имеет короткую деревянную рукоятку, заканчивающуюся изогнутым лезвием, иногда заточенным с обеих сторон.

.. _weapon--Staff:

Посох (`Staff <https://2e.aonprd.com/Weapons.aspx?ID=12>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Этот длинный кусок дерева может помогать при ходьбе и наносить мощные удары.

.. _weapon--Spear:

Копье (`Spear <https://2e.aonprd.com/Weapons.aspx?ID=10>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Длинное металлическое древко, оканчивающееся металлическим шипом, копье может использоваться как одноручное оружие ближнего боя или как метательное.

.. _weapon--Longspear:

Длинное копье (`Longspear <https://2e.aonprd.com/Weapons.aspx?ID=6>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Это очень длинное копье, иногда называемое пикой, предназначено исключительно для колющих ударов, а не для метания.
Используемое многими солдатами и городскими дозорными для контроля толпы и защиты от набегающих врагов, им требуется владеть двумя руками.

.. _weapon--Gauntlet:

Латная рукавица (`Gauntlet <https://2e.aonprd.com/Weapons.aspx?ID=4>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Пара этих металлических перчаток поставляется с полными латами, полулатами и пластинчатым доспехом; их также можно приобрести отдельно и носить с другими видами доспехов.
Они не только защищают ваши руки, но еще превращают ваши руки в смертельное оружие.

.. _weapon--Spiked-Gauntlet:

Шипованая рукавица (`Spiked Gauntlet <https://2e.aonprd.com/Weapons.aspx?ID=11>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Давая такие же защитные функции как и обычная латная рукавица, эта версия имеет набор шипов, выступающих из костяшек пальцев, чтобы при ударе наносить колющий урон.





.. _weapon--Clan-Dagger:

Клановый кинжал (`Clan Dagger <https://2e.aonprd.com/Weapons.aspx?ID=13>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Этот широкий кинжал носится дварфами как оружие, инструмент и обозначение клана.
Потеря или необходимость отдать клановый кинжал считается позором для большинства дварфов.

.. _weapon--Katar:

Катар (`Katar <https://2e.aonprd.com/Weapons.aspx?ID=14>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Также известные как тычковые ножи, катары характеризуются своей H-образной рукоятью, которая позволяет лезвию выступать из суставов пальцев.





.. _weapon--Whip:

Хлыст (`Whip <https://2e.aonprd.com/Weapons.aspx?ID=48>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Эта длинная прядь толстой кожи, часто заплетенная косой, наносит болезненные, но не смертельный удары на расстоянии, обычно сопровождающийся характерным щелчком.

.. _weapon--Sap:

Мягкая дубинка (`Sap <https://2e.aonprd.com/Weapons.aspx?ID=37>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Эта дубинка имеет мягкую оболочку вокруг плотного сердечника, обычно кожаную обертку вокруг свинцового стержня.
Оголовье шире чем рукоятка, чтобы рассеивать силу удара, так как цель оружия - нокаутировать жертву, а не пролить кровь.

.. _weapon--Bo-Staff:

Посох-Бо (`Bo Staff <https://2e.aonprd.com/Weapons.aspx?ID=17>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Этот крепкий, но тонкий посох сужается на концах и хорошо сбалансирован.
Он создан как наступательное и оборонительное оружие.

.. _weapon--Starknife:

Нож-звезда (`Starknife <https://2e.aonprd.com/Weapons.aspx?ID=44>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

От центрального металлического кольца отходят четыре заостренных металлических лезвия, как точки на розе ветров компаса.
При хвате ножа-звезды за центр, владелец может использовать его в качестве оружия ближнего боя.
Он также может быть брошен на короткие расстояния.


.. _weapon--Shield-Bash:

Удар щитом (`Shield Bash <https://2e.aonprd.com/Weapons.aspx?ID=40>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Это не оружие, а маневр при котором вы толкаете или размахиваете своим щитом, чтобы попасть по врагу спонтанной атакой.

.. _weapon--Shield-Boss:

Умбон (`Shield Boss <https://2e.aonprd.com/Weapons.aspx?ID=41>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Обычно круглый, выпуклый или конический кусок толстого металла, прикрепленный к центру щита.
Умбон увеличивает дробящий урон от удара щитом.

.. _weapon--Shield-Spikes:

Шипы для щита (`Shield Spikes <https://2e.aonprd.com/Weapons.aspx?ID=42>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Эти металлические шипы стратегически расположены на защитной стороне щита, чтобы наносить колющий урон с помощью удара щитом.



.. _weapon--Main-gauche:

Мэн-гош (`Main-gauche <https://2e.aonprd.com/Weapons.aspx?ID=32>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Этот кинжал для парирования имеет прочную гарду для защиты руки владельца.

.. _weapon--Shortsword:

Короткий меч (`Shortsword <https://2e.aonprd.com/Weapons.aspx?ID=43>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Эти клинки бывают разных форм и стилей, но они обычно 2 фута длиной.

.. _weapon--Longsword:

Длинный меч (`Longsword <https://2e.aonprd.com/Weapons.aspx?ID=31>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Длинный меч может быть с односторонним клинком или обоюдоострым.
Их клинки тяжелые и длиной между 3 и 4 футами.

.. _weapon--Rapier:

Рапира (`Rapier <https://2e.aonprd.com/Weapons.aspx?ID=36>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Рапира это длинный и тонкий колющий клинок с корзинчатой гардой.
Среди многих, он ценится как дуэльное оружие.

.. _weapon--Scimitar:

Скимитар (`Scimitar <https://2e.aonprd.com/Weapons.aspx?ID=38>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Одноручный, заточеный с одной стороны изогнутый клинок.

.. _weapon--Bastard-Sword:

Меч-бастард (`Bastard Sword <https://2e.aonprd.com/Weapons.aspx?ID=15>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Этот меч с широким клинком, иногда называемый полуторным мечом, имеет более длинную рукоятку, поэтому его можно держать в одной руке или использовать двумя руками, чтобы обеспечить дополнительную рубящую силу.

.. _weapon--Falchion:

Фальшион (`Falchion <https://2e.aonprd.com/Weapons.aspx?ID=18>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Это оружие - утяжеленная двуручная версия скимитара.
Он утяжелен к концу клинка, что делает его мощным рубящим оружием.

.. _weapon--Greatsword:

Двуручный меч (`Greatsword <https://2e.aonprd.com/Weapons.aspx?ID=24>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Этот огромный двуручный меч почти такого же роста, как и его владелец.
Нижняя часть его клинка часто несколько притупляется, чтобы его можно было схватить и использовать как дополнительный упор в ближнем бою.



.. _weapon--Hatchet:

Топорик (`Hatchet <https://2e.aonprd.com/Weapons.aspx?ID=27>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Этот маленький топор можно использовать в ближнем бою или метать.

.. _weapon--Battle-Axe:

Боевой топор (`Battle Axe <https://2e.aonprd.com/Weapons.aspx?ID=16>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Эти топоры специально разработаны как оружие, а не как инструмент.
Они обычно весят меньше, имеют древко усиленное металлическими лентами или болтами, и имеют более острое лезвие, что делает их идеальными для рубки конечностей, а не дерева.

.. _weapon--Greataxe:

Двуручный топор (секира) (`Greataxe <https://2e.aonprd.com/Weapons.aspx?ID=21>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Этот большой боевой топор слишком тяжел, чтобы владеть им лишь одной рукой.
Многие двуручные топоры имеют два лезвия, и они часто бородовидные, имеющие зацеп внизу, для увеличения рубящей силы.



.. _weapon--Light-Pick:

Легкий клевец (`Light Pick <https://2e.aonprd.com/Weapons.aspx?ID=30>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Легкий клевец это модифицированный инструмент горной добычи, с деревянной рукоятью заканчивающейся навершием кирки, изготовленной больше для того, чтобы пробивать доспехи и плоть, а не колоть камни.

.. _weapon--Pick:

Клевец (`Pick <https://2e.aonprd.com/Weapons.aspx?ID=34>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Клевец, предназначенный исключительно для боя, имеет прочную деревянную рукоять и тяжелое заостренное навершие для нанесения сокрушительных ударов.

.. _weapon--Greatpick:

Двуручный клевец (`Greatpick <https://2e.aonprd.com/Weapons.aspx?ID=23>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Этот клевец имеет длинную рукоять и навершие, шире, чем у обычного клевца.
Он слишком тяжелый, чтобы владеть им одной рукой.



.. _weapon--Glaive:

Глефа (`Glaive <https://2e.aonprd.com/Weapons.aspx?ID=20>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Это древковое оружие состоит из длинного однолезвийного клинка на конце 7-футового шеста.
Оно чрезвычайно эффективно при нанесении смертельных порезов на расстоянии.

.. _weapon--Halberd:

Алебарда (`Halberd <https://2e.aonprd.com/Weapons.aspx?ID=26>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Это древковое оружие относительное короткое, имея 5-футовое древко.
Ее рабочий конец - длинный шип с прикрепленным лезвием топора.

.. _weapon--Guisarme:

Гвизарма (`Guisarme <https://2e.aonprd.com/Weapons.aspx?ID=25>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Это длинное древковое оружие, часто с изогнутым лезвием на одной стороне и крюком на обухе, который позволяет владельцу опрокидывать оппонентов на расстоянии.
Его древко обычно 8 футов длиной.

.. _weapon--Ranseur:

Рунка (`Ranseur <https://2e.aonprd.com/Weapons.aspx?ID=35>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Это древковое оружие - длинный трезубец с центральным зубцом, который длиннее двух других.

.. _weapon--Scythe:

Коса (`Scythe <https://2e.aonprd.com/Weapons.aspx?ID=39>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Произошедшее из сельскохозяйственного орудия, используемого для скашивания длинных зерновых и злаков, это оружие имеет длинное деревянное древко с выступающими ручками, увенчанное изогнутым лезвием, установленным под прямым углом.

.. _weapon--Lance:

Кавалерийская пика (`Lance <https://2e.aonprd.com/Weapons.aspx?ID=28>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Это копьеобразное оружие используется существом, верхом на ездовом животном, для нанесения большого урона.

.. _weapon--Trident:

Трезубец (`Trident <https://2e.aonprd.com/Weapons.aspx?ID=45>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Это трехконечное копьеобразное оружие обычно имеет 4-футовое древко.
Как и копьем, им можно владеть одной рукой или метать.



.. _weapon--Flail:

Цеп (`Flail <https://2e.aonprd.com/Weapons.aspx?ID=19>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Это оружие состоит из деревянной рукояти и соединенным цепью, веревкой или кожаными ремнями шипастым шаром или цилиндром.

.. _weapon--War-Flail:

Двуручный цеп (`War Flail <https://2e.aonprd.com/Weapons.aspx?ID=46>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Этот большой цеп имеет длинную рукоять, соединенную с более коротким куском крепкого дерева или металла, который иногда дополнен шипами.



.. _weapon--Light-Hammer:

Легкий молот (`Light Hammer <https://2e.aonprd.com/Weapons.aspx?ID=29>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Эта уменьшенная версия боевого молота имеет деревянную или металлическую рукоять, заканчивающуюся металлическим навершием.
В отличие от своего более тяжелого кузена, он достаточно легок для метания.

.. _weapon--Warhammer:

Боевой молот (`Warhammer <https://2e.aonprd.com/Weapons.aspx?ID=47>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Это оружие имеет деревянную рукоять, заканчивающуюся большим тяжелым металлическим навершием.
Навершие молота может быть односторонним или двусторонним, но оно всегда способно наносить мощные дробящие удары.

.. _weapon--Maul:

Кувалда (`Maul <https://2e.aonprd.com/Weapons.aspx?ID=33>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Кувалды это массивные боевые молоты, которыми нужно размахивать двумя руками.

.. _weapon--Greatclub:

Двуручная дубина (`Greatclub <https://2e.aonprd.com/Weapons.aspx?ID=22>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

В то время как многие двуручные дубины покрыты замысловатой резьбой, другие - не более чем крепкая ветка дерева.
Эти массивные дубины слишком тяжелы, чтобы владеть ими только одной рукой.





.. _weapon--Elven-Curve-Blade:

Эльфийская сабля (`Elven Curve Blade <https://2e.aonprd.com/Weapons.aspx?ID=50>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

По сути, это удлиненная версия скимитара, традиционное эльфийское оружие имеет более тонкий клинок, чем его двоюродный брат.

.. _weapon--Filchers-Fork:

Вилка жулика (`Filcher's Fork <https://2e.aonprd.com/Weapons.aspx?ID=51>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Это оружие полуросликов выглядит как длинная вилка с двумя зубцами, и используется как оружие и кухонная принадлежность.

.. _weapon--Gnome-Hooked-Hammer:

Гномий чекан (`Gnome Hooked Hammer <https://2e.aonprd.com/Weapons.aspx?ID=52>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Этот гномий инструмент и оружие имеет черты молотка на одном конце и изогнутый клевец на другом.
Это настолько странное и неуклюжее оружие, что другие думают, будто гномы немного неустойчивы, чтобы им пользоваться.

.. _weapon--Dogslicer:

Собачий тесак (`Dogslicer <https://2e.aonprd.com/Weapons.aspx?ID=49>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Этот короткий, изогнутый и грубый самодельный клинок часто имеет просверленные в нем отверстия, для уменьшения веса.
Это предпочитаемое оружие гоблинов.

.. _weapon--Horsechopper:

Лошадиный тесак (`Horsechopper <https://2e.aonprd.com/Weapons.aspx?ID=53>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Созданное гоблинами, чтобы сражаться с конями, это оружие представляет из себя длинное древко, заканчивающееся клинком с большим крюком.

.. _weapon--Orc-Knuckle-Dagger:

Орочий кинжал-кастет (`Orc Knuckle Dagger <https://2e.aonprd.com/Weapons.aspx?ID=58>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Этот крепкий металлический клинок орочьей конструкции имеет горизонтальную корзинчатую рукоять с лезвиями, выступающими с обоих концов, а иногда и одно лезвие, как у катара.

.. _weapon--Nunchaku:

Нунчаки (`Nunchaku <https://2e.aonprd.com/Weapons.aspx?ID=57>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Нунчаки состоят из двух деревянных или металлических брусков, соединенных короткой веревкой или цепью.

.. _weapon--Sai:

Сай (`Sai <https://2e.aonprd.com/Weapons.aspx?ID=59>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Этот колющий кинжал представляет собой металлический шип, окруженный парой зубцов, которые могут использоваться чтобы ловить оружие противника.

.. _weapon--Temple-Sword:

Храмовый меч (`Temple Sword <https://2e.aonprd.com/Weapons.aspx?ID=61>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Этот тяжелый клинок предпочитают стражи религиозных мест.
У него есть характерный клинок в форме полумесяца, который кажется смесью серпа и меча.
Он часто имеет отверстия, просверленные в лезвии или навершии, так что к нему могут быть прикреплены колокольчики или другие священные безделушки, в качестве помощи для молитвы или медитации.

.. _weapon--Kama:

Кама (`Kama <https://2e.aonprd.com/Weapons.aspx?ID=54>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Похожая на серп и используемая в некоторых регионах для сбора зерна, кама имеет короткое, слегка изогнутое лезвие и деревянную рукоятку.

.. _weapon--Katana:

Катана (`Katana <https://2e.aonprd.com/Weapons.aspx?ID=55>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Катана - изогнутый, односторонний меч, известный своим опасно острым лезвием. 

.. _weapon--Kukri:

Кукри (`Kukri <https://2e.aonprd.com/Weapons.aspx?ID=56>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Лезвие этого ножа длиной в фут изгибается внутрь и лишено поперечной гарды на рукояти.

.. _weapon--Spiked-Chain:

Шипастая цепь (`Spiked Chain <https://2e.aonprd.com/Weapons.aspx?ID=60>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Эта 4-футовая цепь покрыта зазубринами и имеет шипы на одном или обоих концах.
Некоторые из них имеют металлические обручи, используемые в качестве рукоятей.





.. _weapon--Sawtooth-Saber:

Зазубренная сабля (`Sawtooth Saber <https://2e.aonprd.com/Weapons.aspx?ID=65>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Фирменное оружие убийц "Красных богомолов", это изогнутое лезвие зазубрено, как пила, отсюда и название.

.. _weapon--Dwarven-Waraxe:

Дварфийская секира (`Dwarven Waraxe <https://2e.aonprd.com/Weapons.aspx?ID=62>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Это любимое оружие дварфов имеет большую, богато украшенную верхнюю часть, установленную на толстой рукояти.
Этой мощной секирой можно владеть одной или двумя руками.

.. _weapon--Gnome-Flickmace:

Гномья цепная булава (`Gnome Flickmace <https://2e.aonprd.com/Weapons.aspx?ID=63>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Будучи скорее цепом, чем булавой, это оружие имеет короткую рукоятку, прикрепленную к длинной цепи с шаром на конце.
С помощью замаха запястья, шар устремляется во врага, а после удара, импульс возвращает шар обратно к владельцу.

.. _weapon--Orc-Necksplitter:

Орочий шееруб (`Orc Necksplitter <https://2e.aonprd.com/Weapons.aspx?ID=64>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Этот однолезвийный бородовидный топор имеет зазубренный клинок, который идеально подходит для отделения кости от сухожилия и хряща.





.. _weapon--Hand-Crossbow:

Ручной арбалет (`Hand Crossbow <https://2e.aonprd.com/Weapons.aspx?ID=69>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Иногда называемый плутами или головорезами "переулком", этот маленький арбалет стреляет маленькими болтами, которые иногда используют для отравления цели.
Он достаточно мал, чтобы стрелять одной рукой, но все еще требует двух рук для перезарядки.

.. _weapon--Crossbow:

Арбалет (`Crossbow <https://2e.aonprd.com/Weapons.aspx?ID=67>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Это дальнобойное оружие имеет механизм, похожий на лук, установленный на раме с ручкой, называемой ложе.
Ложе имеет механизм фиксации тетивы, прикрепленный к спусковому крючку, который высвобождает тетиву и запускает болт.

.. _weapon--Heavy-Crossbow:

Тяжелый арбалет (`Heavy Crossbow <https://2e.aonprd.com/Weapons.aspx?ID=70>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Этот большой арбалет труднее заряжать и он более весомый, чем обычный арбалет, но он имеет большую убойную силу.

.. _weapon--Sling:

Праща (`Sling <https://2e.aonprd.com/Weapons.aspx?ID=72>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Это кожеток прикрепленный к паре ремней, праща может быть использована для метания гладких камней или пуль на расстоянии.

.. _weapon--Blowgun:

Духовая трубка (`Blowgun <https://2e.aonprd.com/Weapons.aspx?ID=66>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Эта длинная, узкая трубка используется для стрельбы дротиками для духового ружья, используя только силу выдыхаемого воздуха.

.. _weapon--Javelin:

Метательное копье (`Javelin <https://2e.aonprd.com/Weapons.aspx?ID=71>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Это тонкое копье хорошо сбалансировано для метания, но не предназначено для ближнего боя.

.. _weapon--Dart:

Дротик (`Dart <https://2e.aonprd.com/Weapons.aspx?ID=68>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Это метательное оружие больше стрелы, но короче метательного копья.
Оно обычно имеет короткое деревянное древко, заканчивающееся металлическим наконечником и иногда стабилизируется перьями или мехом.





.. _weapon--Alchemical-Bomb:

Алхимическая бомба (`Alchemical Bomb <https://2e.aonprd.com/Weapons.aspx?ID=73>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Эти бомбы бывают разных типов и уровней мощности, но независимо от разнообразия, вы бросаете бомбу в цель и она взрывается, высвобождая свой алхимический взрыв.

.. _weapon--Shortbow:

Короткий лук (`Shortbow <https://2e.aonprd.com/Weapons.aspx?ID=77>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Этот небольшой лук сделан из цельного куска дерева и пользуется популярностью у застрельщиков и кавалеристов.

.. _weapon--Longbow:

Длинный лук (`Longbow <https://2e.aonprd.com/Weapons.aspx?ID=76>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Этот 5-футовый лук, обычно сделанный из цельного куска вяза, гикори или тиса, имеет мощное натяжение и отлично выпускает стрелы на огромные расстояния.
Вы должны использовать две руки, чтобы стрелять и он не может быть использован верхом на скакуне.

.. _weapon--Composite-Shortbow:

Композитный короткий лук (`Composite Shortbow <https://2e.aonprd.com/Weapons.aspx?ID=75>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Этот короткий лук сделан из рога, дерева и сухожилий, соединенных вместе, чтобы увеличить мощность натяжения и силу его снаряда.
Его компактный размер и мощность, делают его любимым оружием лучников на скакунах.
Всякий раз, когда умение ограничено конкретно коротким луком, оно также применяется к композитным коротким лукам, если не сказано иначе.

.. _weapon--Composite-Longbow:

Композитный длинный лук (`Composite Longbow <https://2e.aonprd.com/Weapons.aspx?ID=74>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Это дистанционное оружие сделано из рога, дерева и сухожилий, соединенных вместе, чтобы увеличить мощность натяжения и силу его снаряда.
Как и у всех длинных луков, его большой размер увеличивает дальность и мощность.
Чтобы стрелять, вы должны использовать две руки и он не может быть использован верхом на скакуне.
Всякий раз, когда умение ограничено конкретно длинным луком, как например любимое оружие :ref:`Эрастила <Deity--Erastil>`, оно также применяется к композитным длинным лукам, если не сказано иначе.





.. _weapon--Halfling-Sling-Staff:

Посох-праща полуросликов (`Halfling Sling Staff <https://2e.aonprd.com/Weapons.aspx?ID=78>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Этот посох заканчивается Y-образным разделением для пращи.
Длина посоха обеспечивает отличный рычаг воздействия при использовании его двумя руками для метания камней или пуль из пращи.

.. _weapon--Shuriken:

Сюрикен (`Shuriken <https://2e.aonprd.com/Weapons.aspx?ID=79>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Эта "метательная звездочка" представляет собой небольшой плоский кусок металла с острыми краями, предназначенный для броска одним движением запястья.





.. _weapon--Arrow:

Стрела (`Arrow <https://2e.aonprd.com/Weapons.aspx?ID=83>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Эти снаряды являются амуницией для луков.
Древко стрелы сделано из дерева.
Она стабилизируется в полете благодаря оперению на одном конце и имеет металлический наконечник на другом.

.. _weapon--Bolt:

Болт (`Bolt <https://2e.aonprd.com/Weapons.aspx?ID=81>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Короче традиционных стрел, но похожи по конструкции, болты являются амуницией для арбалетов.

.. _weapon--Sling-Bullets:

Пули для пращи (`Sling Bullets <https://2e.aonprd.com/Weapons.aspx?ID=82>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Это маленькие металлические шарики, обычно из железа или свинца, изготовленные для использования как амуниция к праще.

.. _weapon--Blowgun-Darts:

Дротики духового ружья (`Blowgun Darts <https://2e.aonprd.com/Weapons.aspx?ID=80>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Эти тонкие, легкие дротики, как правило, сделаны из твердой древесины и стабилизированы с помощью оперения из пуха или меха.
Они часто полые внутри и могут использоваться для отравления.










.. Advanced Player's Guide (APG)

.. _weapon--Bola:

Бола (`Bola <https://2e.aonprd.com/Weapons.aspx?ID=92>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: Advanced Player's Guide pg. 248, Lost Omens: Gods & Magic pg. 120

Это метательное оружие состоит из гирь, привязанных к концу длинных шнуров, которые могут быть использованы чтобы бить врагов или опутывать их ноги.

.. _weapon--Claw-Blade:

Когтистый клинок (`Claw Blade <https://2e.aonprd.com/Weapons.aspx?ID=124>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: Advanced Player's Guide pg. 248

Три параллельных лезвия этого ручного оружия проходят между пальцами, чтобы напоминать естественные когти создавших их амуррунов, давая представителю кошачьего народа без подходящих естественных когтей, возможность разделить боевые обычаи своих сородичей.

.. _weapon--Daikyu:

Дайкю (`Daikyu <https://2e.aonprd.com/Weapons.aspx?ID=125>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: Advanced Player's Guide pg. 248

Этот асимметричный лук, сделанный из слоистого бамбука, дерева и кожи, имеет высоту 6 футов или более.
Использование дайкю на скаку ограничивает его дальность стрельбы с левой стороны.

.. _weapon--Khakkara:

Хаккара (`Khakkara <https://2e.aonprd.com/Weapons.aspx?ID=126>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: Advanced Player's Guide pg. 248

Этот посох увенчан заостренным металлическим кругом, с которого свисают несколько небольших колец, которые громко звенят и лязгают при перемещении посоха, позволяя вам объявить о своем присутствии и отпугнуть диких животных, когда вы идете.

.. _weapon--Sword-Cane:

Меч-трость (`Sword Cane <https://2e.aonprd.com/Weapons.aspx?ID=127>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: Advanced Player's Guide pg. 248

Этот тонкий, похожий на рапиру меч спрятан внутри деревянной или металлической трости, которая служит ножнами, что делает его неприметным оружием, которое может легко проскользнуть мимо проверок или на светские мероприятия.
Меч-трость обычно имеет длину 4 фута, когда вложен в ножны, и его рукоять обычно украшена деревянным или металлическим украшением.

.. _weapon--Tengu-Gale-Blade:

Штормовой клинок тэнгу (`Tengu Gale Blade <https://2e.aonprd.com/Weapons.aspx?ID=128>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: Advanced Player's Guide pg. 248

Этот веерообразный меч, разработанный представителями народа тэнгу, имеет пять широких лезвий, которые соединяются на его рукояти.
Моряки тэнгу также используют штормовые клинки в качестве импровизированных флюгеров, поскольку меч вращается, чтобы показать направление ветра, когда он свободно держится в воздухе.

.. _weapon--Wakizashi:

Вакидзаси (`Wakizashi <https://2e.aonprd.com/Weapons.aspx?ID=129>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: Advanced Player's Guide pg. 248

Этот короткий необоюдоострый клинок обычно носят в паре с катаной.





.. Gears

.. _weapon--Backpack-Ballista:

Рюкзак-баллиста (`Backpack Ballista <https://2e.aonprd.com/Weapons.aspx?ID=173>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: Guns & Gears pg. 62

Это сложное деревянное устройство, носимое на спине, содержит миниатюрную баллисту на выдвижном рычаге.
Действием :ref:`action--Interact` вы можете потянуть за рычаг, чтобы развернуть или убрать баллисту.
Пока она развернута, вы должны удерживать баллисту этой рукой, иначе некоторые компоненты высыпятся на землю, как если бросаете любое другое оружие.
В развернутом состоянии устройство открывается и поднимает баллисту над вашим плечом.
В убранном состоянии баллиста и ее крепление спускается вниз и скрыто внутри устройства.
Несмотря на то, что рюкзак-баллиста обладает мощным выстрелом, устройство является сложным в эксплуатации.
Перезарядка занимает 1 минуту и не может быть выполнена во время ношения.
Как обычно, вы не можете носить рюкзак-баллисту с другим рюкзаком.

.. _weapon--Backpack-Catapult:

Рюкзак-катапульта (`Backpack Catapult <https://2e.aonprd.com/Weapons.aspx?ID=174>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: Guns & Gears pg. 63

Это деревянное устройство носится на спине и содержит миниатюрную катапульту, установленную на выдвижной раме.
Действием :ref:`action--Interact` вы можете потянуть за рычаг, чтобы развернуть или убрать катапульту.
Пока она находится в развернутом состоянии, вы должны держать катапульту этой рукой, иначе некоторые компоненты высыпятся на землю, как если бросаете любое другое оружие.
В развернутом состоянии устройство открывается и поднимает катапульту над вашим плечом.
В убранном состоянии катапульта и ее крепление спускается вниз и скрываются внутри устройства.
Рюкзак-катапульта стреляет специализированными каменными сферами, которые через сдвижной люк убираются в ковш в ненадетом и убранном состоянии; процесс перезарядки занимает 1 минуту.
Во избежание осечек и случайных травм ковш полностью закрывает камень в развернутом состоянии, открываясь только при выстреле.
Как обычно, вы не можете носить рюкзак-катапульту с другим рюкзаком.

.. _weapon--Dart-Umbrella:

Зонт с дротиками (`Dart Umbrella <https://2e.aonprd.com/Weapons.aspx?ID=175>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: Guns & Gears pg. 63

Этот зонтик при повороте рукоятки стреляет из своего наконечника крошечными духовыми дротиками.
Дротики заряжаются в стержень зонта, и хотя наносимый ими урон минимален, зонт с дротиками - неприметное оружие, которое легко пронести мимо досмотра.

.. _weapon--Umbrella-Injector:

Инъекционный зонт (`Umbrella Injector <https://2e.aonprd.com/Weapons.aspx?ID=171>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: Guns & Gears pg. 64

Металлический наконечник этого зонта представляет собой полый клинок длиной 3-4 дюйма (~ 7.6-10.1 см), который часто принимают за декоративный.
Сосуд внутри стержня зонта может быть наполнен 1 дозой яда вводимого через :t_injury:`ранение` и впрыснут в раненую цель нажатием на скользящий курок.
Усиленные ребра позволяют парировать и отражать удары благодаря устойчивому к разрывам купола зонта.

.. _weapon--Injection-Spear:

Инъекционное копье (`Injection Spear <https://2e.aonprd.com/Weapons.aspx?ID=170>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: Guns & Gears pg. 63

Полый сосуд внутри наконечника этого копья может содержать 1 дозу яда.
Скользящий курок вдоль древка копья может впрыснуть заряженный яд в раненую цель.

.. _weapon--Pantograph-Gauntlet:

Пантографическая рукавица (`Pantograph Gauntlet <https://2e.aonprd.com/Weapons.aspx?ID=172>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: Guns & Gears pg. 63

Пантографическая рукавица - тяжелый груз, похожий на кулак, установленный на выдвижной раме и закрепленный на внешней стороне руки с помощью ряда кожаных ремней.
Набор механических соединений рамы, соединенных различными шарнирами, обеспечивает движение по раме, основанное на изменении формы параллелограммов, которое контролируется поперечиной, находящейся в вашей руке.
Пантографическая рукавица приводится в движение вашим собственным движением и отражает движения вашей руки - удар, нанесенный вашим кулаком, приводит в движение пантограф, с большой скоростью выдвигающий груз для нанесения ударов на расстоянии вплоть до 10 футов.
В некоторых регионах, таких как Алкенстар и Усталав, пантографические рукавицы иногда изготавливают полностью из металла и делают в виде огромных рук, применяя сложную систему шестерней или миниатюрный паровой двигатель вместо более простого механизма пантографа.

.. _weapon--Repeating-Hand-Crossbow:

Многозарядный ручной арбалет (`Repeating Hand Crossbow <https://2e.aonprd.com/Weapons.aspx?ID=177>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: Guns & Gears pg. 64, Pathfinder #165: Eyes of Empty Death pg. 73

Этот ручной арбалет оснащен хитроумно разработанным механизмом взведения в верхней части направляющей, прямо перед замком, который автоматически заряжает болт из магазина и натягивает тетиву после каждого выстрела.
Типичный магазин многозарядного ручного арбалета содержит 5 болтов.

.. _weapon--Repeating-Crossbow:

Многозарядный арбалет (`Repeating Crossbow <https://2e.aonprd.com/Weapons.aspx?ID=176>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: Guns & Gears pg. 64

Этот арбалет имеет внутреннюю камеру, в которую можно зарядить вплоть до 5 болтов.
Автоматический механизм взведения в верхней части направляющей, непосредственно перед замком, фиксирует тетиву и запускает болты при нажатии на курок.

.. _weapon--Repeating-Heavy-Crossbow:

Многозарядный тяжелый арбалет (`Repeating Heavy Crossbow <https://2e.aonprd.com/Weapons.aspx?ID=178>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: Guns & Gears pg. 64

Этот большой арбалет имеет внутреннюю камеру, в которую можно зарядить вплоть до 5 болтов.
Хотя в нем используется тот же автоматический механизм взведения, что и в других многозарядных арбалетах, конструкция такого тяжелого арбалета имеет существенные недостатки.
Он имеет увеличенную дальность стрельбы и урон, им легче научиться пользоваться, но для перезарядки требуется небольшое усилие.

.. _weapon--Repeating-Crossbow-Magazine:

Магазин многозарядного арбалета (`Repeating Crossbow Magazine <https://2e.aonprd.com/Weapons.aspx?ID=181>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник:** Guns & Gears pg. 64

Типичный магазин для многозарядного арбалета вмещает вплоть до 5 болтов.
Бывают для обычных, ручных и тяжелых многозарядных арбалетов.

**Примечание**: Так как характеристики (Цена, вес, вместимость) разновидностей магазинов одинаковые, здесь для простоты описан "сферический арбалетный магазин в вакууме", но имейте в виду, что у каждого вида многозарядного арбалета он свой.





.. Guns

.. _weapon--Air-Repeater:

Воздушная винтовка (`Air Repeater <https://2e.aonprd.com/Weapons.aspx?ID=188>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: Guns & Gears pg. 152

Тонкоствольное огнестрельное оружие, использующее вместо черного пороха баллон со сжатым воздухом, чтобы стрелять маленькими металлическими пулями из прикрепленного патрона.
Воздушная винтовка вышла из широкого употребления в Аркадии из-за своей слабой останавливающей силы, хотя она все еще иногда используется для любительской охоты и спортивной стрельбы.
Воздушная винтовка и ее дальнобойный двуручный вариант все еще ценятся некоторыми за возможность, позволяющую стрелку произвести несколько выстрелов без необходимости останавливаться для перезарядки или досылания нового патрона.
Типичный магазин воздушной винтовки вмещает 6 пулек.

.. _weapon--Arquebus:

Аркебуза (`Arquebus <https://2e.aonprd.com/Weapons.aspx?ID=195>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: Guns & Gears pg. 152

Это длинная винтовка, обеспечивающая бо́льшую дальность стрельбы, чем обычное огнестрельное оружие, однако длинный ствол и дикая отдача делают это оружие особенно неустойчивым, если только для его стабилизации не используется штатив или другая подставка.
Аркебуза - одно из самых распространенных длинноствольных оружий среди солдат Донган Холда и Алкенстара.

.. _weapon--Bayonet:

Штык (`Bayonet <https://2e.aonprd.com/Weapons.aspx?ID=186>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: Guns & Gears pg. 152

Этот клинок или шип можно прикрепить к арбалету или огнестрельному оружию, но, в отличие от другого прикрепляемого оружия, им можно владеть одной рукой как самостоятельным оружием.
Когда он используется как отдельное оружие, на него не действуют никакие руны или умения, предназначенные только для прикрепляемого оружия.

.. _weapon--Blunderbuss:

Мушкетон (`Blunderbuss <https://2e.aonprd.com/Weapons.aspx?ID=196>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: Guns & Gears pg. 152

Это оружие стреляет дробью из ствола в форме трубы, что делает его отличным выбором для охоты на пернатую дичь или нанесения урона вблизи по широкой области.
Стрелки-авантюристы часто носят с собой мушкетон для борьбы с :t_swarm:`роями` паразитов и тому подобными угрозами.

.. _weapon--Clan-Pistol:

Клановый пистолет (`Clan Pistol <https://2e.aonprd.com/Weapons.aspx?ID=197>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: Guns & Gears pg. 152

Традиция дварфов демонстрировать свою принадлежность к клану с помощью специальных клановых кинжалов насчитывает тысячелетия, но многие дварфийские кланы Донган Холда имеют свой собственный подход к этой традиции:
перспективные молодые оружейники заявляют о своем взрослении, создавая специальное личное огнестрельное оружие с использованием уникальных кузнечных традиций клана.
Потеря или вынужденная сдача кланового пистолета - страшный позор для носящих их дварфов.

.. _weapon--Coat-Pistol:

Карманный пистолет (`Coat Pistol <https://2e.aonprd.com/Weapons.aspx?ID=189>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: Guns & Gears pg. 152

Этот небольшой пистолет легко спрятать в куртке или другом предмете одежды.
Карманные пистолеты редко используют в качестве основного оружия, их одинаково предпочитают ловкие убийцы и путешествующие Алкенстарские аристократы.

.. _weapon--Double-Barreled-Musket:

Двуствольный мушкет (`Double-Barreled Musket <https://2e.aonprd.com/Weapons.aspx?ID=198>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: Guns & Gears pg. 152

Этот кремневый мушкет с казёнником имеет 2 ствола, расположенных двуг возле друга.
Хотя двуствольный мушкет менее точен, по сравнению с обычным мушкетом, он обеспечивает универсальность вариантов стрельбы.
Многие знаменитые шилд-маршалы Алкенстара откладывают свои сбережения, чтобы купить двуствольный мушкет в качестве первого личного оружия.

.. _weapon--Double-Barreled-Pistol:

Двуствольный пистолет (`Double-Barreled Pistol <https://2e.aonprd.com/Weapons.aspx?ID=199>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: Guns & Gears pg. 152

Этот кремневый пистолет имеет 2 ствола, расположенных двуг возле друга.
Хотя двуствольный пистолет менее точен, по сравнению с обычным пистолетом, он является полезным и универсальным оружием.
Он обычно запрещен в регионах, где пистолетные дуэли относительно распространены, точно так же, как прибытие на дуэль аристократов с ружьем считается в лучшем случае грубостью.

.. _weapon--Dragon-Mouth-Pistol:

Пистолет драконьей пасти (`Dragon-Mouth Pistol <https://2e.aonprd.com/Weapons.aspx?ID=200>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: Guns & Gears pg. 152

Подобно мушкетону, пистолет драконьей пасти стреляет дробью из расширяющегося ствола.
Хотя этот пистолет не такой мощный, как мушкетон, его ценят за портативность и одноручный дизайн.
Хотя название было придумано из-за разрушительной отрыжки этого ручного :w_scatter:`рассеивающего` оружия, многие оружейники делают пистолеты драконьей пасти с замысловатыми украшениями, напоминающими стилизованную пасть дракона, обрамляющую ствол.

.. _weapon--Dueling-Pistol:

Дуэльный пистолет (`Dueling Pistol <https://2e.aonprd.com/Weapons.aspx?ID=201>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: Guns & Gears pg. 152

Созданные для разрешения споров, когда дипломатия терпит неудачу, дуэльные пистолеты отлично изготовлены и легко помещаются в кобуру или карман.
Знатные и богатые купцы как в Алкенстаре, так и в Донган Холде часто владеют парами дуэльных пистолетов на случай, если их вызовут на дуэль, однако в современную эпоху эта практика становится все более редкой.

.. _weapon--Dwarven-Scattergun:

Дварфийское ружье (`Dwarven Scattergun <https://2e.aonprd.com/Weapons.aspx?ID=207>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: Guns & Gears pg. 153

Излюбленное оружие дварфов-разведчиков из Донган Холда, дварфийское ружье - мощное оружие разработанное, чтобы использовать преимущества крепкого телосложения дварфа.
Дварфийское ружье стреляет большим бумажным патроном, начиненным черным порохом и кусками металла размером с костяшки пальцев, создавая огромный взрыв, настолько разрушительный, что глупый дварф может словить болезненные рикошеты при стрельбе по слишком близкой цели.
Некоторые ружья имеют лоток с заводным ударно-механизмом, который позволяет быстро сортировать и загружать черный порох и дробь без необходимости готовых патронов, хотя это в основном эстетическая особенность, без каких-либо механических преимуществ.

.. _weapon--Fire-Lance:

Огненное копье (`Fire Lance <https://2e.aonprd.com/Weapons.aspx?ID=190>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: Guns & Gears pg. 153

Это удивительно простое метательное оружие представляет собой не что иное, как металлическую трубку, набитую черным порохом и пробкой, прикрепленную к заостренному наконечнику копья.
Заряженным огненным копьем можно владеть как обычным копьем, однако чтобы перехватить оружие и правильно держать его при переходе от одного использования к другому, требуется действие :ref:`action--Interact`.
Огненные копья чаще всего встречаются в Тянь Ся, хотя порой одно из них добирается до Авистана, обычно в руках тиянских наемников или охранников караванов.

.. _weapon--Flingflenser:

Швыродёр (`Flingflenser <https://2e.aonprd.com/Weapons.aspx?ID=208>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: Guns & Gears pg. 153

Швыродёр - оружие гоблинской разработки, заканчивающееся яйцевидной трубкой с люком и рукояткой на узком конце.
В качестве боеприпасов используется скопление круглых лезвий, скрепленных вместе и прикрепленных тонким кожаным ремешком к пачке с черным порохом, который перед выстрелом заряжается через люк, поджигаясь ударно-кремневым замком или другим внешним спусковым механизмом.
Прочная конструкция швыродёра также делает его одним из самых надежных гоблинских оружий.

.. _weapon--Flintlock-Musket:

Кремневый мушкет (`Flintlock Musket <https://2e.aonprd.com/Weapons.aspx?ID=191>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: Guns & Gears pg. 153

Наиболее распространенный из огнестрельного оружия в Алкенстаре, кремневый мушкет имеет внешний спусковой механизм и, эффективную и относительно компактную ствольную коробку.
Хотя кремневый мушкет не обладает такой дальнобойностью и огневой мощью аркебузы, которую предпочитают военные Алкенстара и Донган Холда, благодаря простоте использования он популярен среди гражданского населения.

.. _weapon--Flintlock-Pistol:

Кремневый пистолет (`Flintlock Pistol <https://2e.aonprd.com/Weapons.aspx?ID=192>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: Guns & Gears pg. 153

Хотя кремневый пистолет менее точный и мощный, чем кремневый мушкет, он является предпочтительным оружием каперов благодаря более компактному размеру и доступности.
Пиратские капитаны часто носят в бандольере кучу таких пистолетов, чтобы их можно было достать и выстрелить, не останавливаясь для перезарядки.

.. _weapon--Forked-Bipod:

Вильчатая сошка (`Forked Bipod <https://2e.aonprd.com/Weapons.aspx?ID=184>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: Guns & Gears pg. 153

Разработанное Алкенстарскими разведчиками, у которых часто не хватает времени, чтобы поставить треногу, когда их преследуют хаотичные монстры-мутанты с Пустошей Маны, это колющее оружие с двумя зубцами может использоваться в качестве сошек для стабилизации оружия с мощной отдачей.
Вильчатая сошка действием :ref:`action--Interact` может быть размещена или убрана для использования в качестве оружия ближнего боя.

.. _weapon--Hand-Cannon:

Ручница (`Hand Cannon <https://2e.aonprd.com/Weapons.aspx?ID=193>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: Guns & Gears pg. 153

Популярные среди каперов и наемников в Гоке, ручни́цы - не более чем закаленная труба с рукояткой и прикрепленным внешним запалом.
Из ручницы можно стрелять почти всем, что помещается в ее ствол.
Владелец ручницы может менять тип наносимого урона, получаемого от признака :w_modular:`модульное`, как часть того же действия :ref:`action--Interact`, которое используется для перезарядки.

.. _weapon--Harmona-Gun:

Ружье-гармона (`Harmona Gun <https://2e.aonprd.com/Weapons.aspx?ID=202>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: Guns & Gears pg. 153

Излюбленное оружие в Аркадии, среди охотников на монстров, гармона - это крупнокалиберное длинноствольное ружье, стреляющее тяжелыми, медленными пулями.
Свое название ружье получило из-за жуткого сходства между жужжащим звуком, который издают огромные снаряды, летящие по воздуху, и полетом фейской птички `гармоны <https://2e.aonprd.com/Monsters.aspx?ID=1190>`_.

.. _weapon--Jezail:

Джезайл (`Jezail <https://2e.aonprd.com/Weapons.aspx?ID=203>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: Guns & Gears pg. 153

Джезайлы - простые, эффективные длинноствольные ружья, разработанные в Касмароне, которые обычно имеют особое ложе и ударно-кремневый механизм.
Хотя джезайл не обладает такой дальнобойностью и останавливающей силой, как аркебуза или ружье-гармона, это элегантное, хорошо сбалансированное оружие, подходящее для различных боевых ситуаций.
Его даже можно зажать подмышкой, чтобы наносить менее точные выстрелы, используя всего 1 руку.

.. _weapon--Knuckle-Duster:

Кастет (`Knuckle Duster <https://2e.aonprd.com/Weapons.aspx?ID=185>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: Guns & Gears pg. 153

Этот кусок закаленного металла, который предпочитают уличные бандиты, обычно изготавливается из латуни и имеет 4 отверстия для пальцев, чтобы его можно было разместить поверх костяшек пальцев, придавая удару дополнительную силу.

.. _weapon--Long-Air-Repeater:

Длинная воздушная винтовка (`Long Air Repeater <https://2e.aonprd.com/Weapons.aspx?ID=194>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: Guns & Gears pg. 153

Как и одноручная воздушная винтовка, это тонкоствольное оружие использует вместо черного пороха баллон со сжатым воздухом, чтобы запускать маленькие металлические пули из прикрепленного патрона.
Длинная воздушная винтовка обладает большей дальность стрельбы и емкостью магазина, чем одноручный вариант, хотя все еще не обладает значительной останавливающей силой.
Типичный магазин длинной воздушной винтовки вмещает 8 пулек.

.. _weapon--Mithral-Tree:

Мифрильное древо (`Mithral Tree <https://2e.aonprd.com/Weapons.aspx?ID=204>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: Guns & Gears pg. 153

И не мифрильное, и не дерево, это длинноствольное оружие получило свое название из легенд эльфов Джинина и наиболее распространено в этой нации.
Мифрильное дерево - элегантное оружие, все же чем-то напоминающее дерево;
его расширенный приклад и длинный, размашистый ствол, усиленный металлическими кольцами, позволяют владельцу парировать атаки ближнего боя, одновременно отходя на безопасную огневую дистанцию.

.. _weapon--Pepperbox:

Перечница (`Pepperbox <https://2e.aonprd.com/Weapons.aspx?ID=205>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: Guns & Gears pg. 153

Это оружие - отличительная черта кузнецов Алкенстара.
Перечница имеет 3 ствола, каждый из которых заряжен на один выстрел, и стрелок может вручную поворачивать весь ствольный механизм, чтобы совместить заряженный ствол со спусковым механизмом.

.. _weapon--Reinforced-Stock:

Усиленный приклад (`Reinforced Stock <https://2e.aonprd.com/Weapons.aspx?ID=187>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: Guns & Gears pg. 153

Усиленный приклад это утяжеление арбалета, либо ложа огнестрельной винтовки или рукояти пистолета, приспособленные для нанесения ударов в ближнем бою.

.. _weapon--Slide-Pistol:

Скользящий пистолет (`Slide Pistol <https://2e.aonprd.com/Weapons.aspx?ID=206>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: Guns & Gears pg. 153

Это оружие, также известное как пистолет-гармошка, по сути, представляет собой ложе, курок и спусковой механизм, прикрепленные к скользящей скобе со стволами, каждый из которых вмещает по одному заряду амуниции.

.. _weapon--Spoon-Gun:

Ложкострел (`Spoon Gun <https://2e.aonprd.com/Weapons.aspx?ID=225>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: Guns & Gears pg. 159

Никто точно не знает, кто разработал ложкострел, но все авторитеты соглашаются, что это, вероятно, был гоблин.
Это по своей сути ужасная идея получившая форму огнестрельного оружия, ложкострел - это пружинная :ref:`ручница <weapon--Hand-Cannon>` с модифицированной рукоятью, использующая в качестве амуниции различные ножи, вилки, ложки и палочки для еды.
Использующие его, обычно засыпают все содержимое ящика со столовыми приборами в оружие, направляют его в сторону врага и надеются, что она во что-нибудь попадет.

Эта ручница является воинским оружием, а не простым.
Она обладает признаком :w_scatter:`разброс 5 фт` и использует в качестве амуниции не пули, а столовые приборы или предметы похожего размера (стоимость дешевых столовых приборов, достаточных для 10 выстрелов, составляет 1 см).

.. _weapon--Big-Boom-Gun:

Большая бабахалка (`Big Boom Gun <https://2e.aonprd.com/Weapons.aspx?ID=226>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: Guns & Gears pg. 159

Разработанная гоблином-оружейником, который в слове ":ref:`ручница <weapon--Hand-Cannon>`" пропустил часть про "руку", эта пушка комически больших размеров имеет весьма усиленный ствол и заряжается вызывающим опасения количеством пороха.
Эта ручница является воинским оружием, а не простым.
Она обладает признаком :w_fatal:`фатальное d12` и дальностью 20 футов.
Оно также имеет следующий измененный результат критического провала:

**Критический провал**: Атака промахивается, оружие дает осечку и вы получаете 1d12 урона огнем, так как оружие взрывается вам в лицо.


.. _weapon--Axe-Musket:

Мушкет-топор (`Axe Musket <https://2e.aonprd.com/Weapons.aspx?ID=213>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: Guns & Gears pg. 159

Этот предмет, предпочитаемый дварфами и варварами, использующими огнестрельное оружие, имеет форму прочного мушкета с прикрепленным около дула топором.

.. _weapon--Black-Powder-Knuckle-Dusters:

Пороховые кастеты (`Black Powder Knuckle Dusters <https://2e.aonprd.com/Weapons.aspx?ID=214>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: Guns & Gears pg. 159

Эта пара :ref:`кастетов <weapon--Knuckle-Duster>` оснащена взрывным зарядом черного пороха внутри полых шипов у оружия и спусковым механизмом, который вы держите в руке во время использования.

.. _weapon--Cane-Pistol:

Пистолет-трость (`Cane Pistol <https://2e.aonprd.com/Weapons.aspx?ID=215>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: Guns & Gears pg. 159

В рукоятке этой модной трости спрятан дуэльный пистолет, стреляющий через тонкий покрашенный колпачок в нижней части трости.

.. _weapon--Dagger-Pistol:

Пистолет-кинжал (`Dagger Pistol <https://2e.aonprd.com/Weapons.aspx?ID=216>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: Guns & Gears pg. 159

Это оружие, предпочитаемое рейнджерами и другими странниками дикой местности, выполнено в форме прочного пистолета с кинжальным лезвием, закрепленным под стволом и выходящим за пределы дула.

.. _weapon--Explosive-Dogslicer:

Взрывной собачий тесак (`Explosive Dogslicer <https://2e.aonprd.com/Weapons.aspx?ID=224>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: Guns & Gears pg. 159

Взрывной собачий тесак - это коварное, взрывное оружие, которое часто приносит извращенную радость использующим его гоблинам.
На первый взгляд, это трехлезвийный :ref:`weapon--Dogslicer` с огромной гардой.

.. _weapon--Gnome-Amalgam-Musket:

Гномий амальгамный мушкет (`Gnome Amalgam Musket <https://2e.aonprd.com/Weapons.aspx?ID=217>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: Guns & Gears pg. 159

По слухам, это оружие было результатом смелости гномов создать еще более замысловатый и сложный вариант :ref:`гномьего чекана <weapon--Gnome-Hooked-Hammer>`, оно добавляет :ref:`мушкет <weapon--Flintlock-Musket>` к уже и без того переусложненному устройству.

.. _weapon--Gun-sword:

Ружье-меч (`Gun sword <https://2e.aonprd.com/Weapons.aspx?ID=218>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: Guns & Gears pg. 159

Это оружие состоит из большого меча с мощным ружьем на основе :ref:`ружья-гармоны <weapon--Harmona-Gun>` в центре.
:ref:`Авангарды <class-feature--Gunslinger--Way--Vanguard>` и другие персонажи, полагающиеся на Силу и Ловкость, наслаждаются мощью и гибкостью ружья-меча.

.. _weapon--Hammer-Gun:

Ружье-молот (`Hammer Gun <https://2e.aonprd.com/Weapons.aspx?ID=219>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: Guns & Gears pg. 159

Это оружие, предпочитаемое дварфами и любителями колотить и стрелять, имеет форму крепко сконструированного ружья, по дизайну похожего на :ref:`аркебузу <weapon--Arquebus>`, со встроенным в дуло навершием молота, уменьшающим отдачу, но ограничивающим дальность стрельбы.

.. _weapon--Mace-Multipistol:

Булава-мультипистолет (`Mace Multipistol <https://2e.aonprd.com/Weapons.aspx?ID=220>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: Guns & Gears pg. 159

На первый взгляд это оружие выглядит не более чем окованной железом дубиной.
Но в верхней части оружия имеется защелка, открыв которую, можно увидеть 3 вращающихся пистолетных дула, спрятанных в навершии :ref:`булавы <weapon--Mace>`, которые могут стрелять и вращаться с помощью курков, встроенных в рукоять оружия.

.. _weapon--Piercing-Wind:

Пронзающий ветер (`Piercing Wind <https://2e.aonprd.com/Weapons.aspx?ID=221>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: Guns & Gears pg. 159

Предпочитаемый охранниками караванов, проходящих через Пустоши Маны, пронзающий ветер похож на :ref:`weapon--Jezail` тем, что его можно носить в одной руке, зажав подмышкой, держа вторую свободной.
Кроме того, он оснащен изогнутым подствольным клинком.

.. _weapon--Rapier-Pistol:

Пистолет-рапира (`Rapier Pistol <https://2e.aonprd.com/Weapons.aspx?ID=222>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: Guns & Gears pg. 159

Это элегантное оружие по форме напоминает :ref:`рапиру <weapon--Rapier>` с пистолетом по всей длине клинка.

.. _weapon--Three-Peaked-Tree:

Трехпиковое древо (`Three Peaked Tree <https://2e.aonprd.com/Weapons.aspx?ID=223>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: Guns & Gears pg. 159

Недавно разработанное оружие, созданное для эльфийского чемпиона из Джинина, трехпиковое дерево может использоваться как :ref:`weapon--Trident` и как :ref:`weapon--Mithral-Tree`.
К прикладу трехпикового древа прикреплена шелковая веревка, позволяющая быстро притягивать его после броска.



.. _weapon--Asp-Coil:

Свернувшийся аспис (`Asp Coil <https://2e.aonprd.com/Weapons.aspx?ID=233>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Доступ**: Агент :ref:`Faction--Aspis-Consortium`

**Источник**: Lost Omens: Grand Bazaar pg. 105

Свернувшийся аспис, обладающий двумя формами, называется так как за свой скользящий стиль нанесения ударов, так и за использование среди агентов :ref:`Консорциума Асписов <Faction--Aspis-Consortium>`.
В форме меча он напоминает элегантный, странно сбалансированный меч.
Однако при повороте рукояти клинок разделяется на сегменты, соединенные сложными металлическими тросами.

.. _weapon--Butchering-Axe:

Потрошащий топор (`Butchering Axe <https://2e.aonprd.com/Weapons.aspx?ID=234>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: Lost Omens: Grand Bazaar pg. 105

Изобретенный ревностными Пожирателями Стали Белкзена, потрошащий топор имеет огромное навершие и длинное толстое древко, уравновешенный сталью или камнем.
Размашистые удары этого оружия наносят огромный урон, особенно против групп врагов, и могут отталкивать опасных противников на безопасное расстояние.
Все эти качества особенно полезны против неуклюжих орд зомби Шепчущего Тирана.
Соответственно, потрошащие топоры часто используют орки и полуорки Багровых Восстановителей Ластволла.

.. _weapon--Chakram:

Чакрам (`Chakram <https://2e.aonprd.com/Weapons.aspx?ID=235>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: Lost Omens: Grand Bazaar pg. 105

Простой, элегантный и портативный чакрам представляет собой металлический диск с полым центром и заостренным краем, а также рукояткой, проходящей через центр, чтобы владелец мог надежно держать его.

.. _weapon--Dueling-Spear:

Дуэльное копье (`Dueling Spear <https://2e.aonprd.com/Weapons.aspx?ID=236>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: Lost Omens: Grand Bazaar pg. 105

Это копье имеет на одном конце лезвие, по форме похожее на штыковую лопату, и раздвоенное лезвие на другом, что делает его похожим на большую стрелу.
Оно хорошо сбалансировано для вращательных и крутящих приемов.
Лопатообразный конец можно использовать для колющих и бьющих ударов, а вилообразный конец эффективен для вырывания оружия из рук противника.

.. _weapon--Elven-Branched-Spear:

Эльфийское ветвящееся копье (`Elven Branched Spear <https://2e.aonprd.com/Weapons.aspx?ID=237>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: Lost Omens: Grand Bazaar pg. 105

От древка этого изящного копья отходят несколько коротких ветвлений, каждое из которых наклонено вперед и оканчивается лезвием похожим на лист.

.. _weapon--Hongali-Hornbow:

Хонгальский роговой лук (`Hongali Hornbow <https://2e.aonprd.com/Weapons.aspx?ID=238>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Доступ**: Персонажи из Хонгала

**Источник**: Lost Omens: Grand Bazaar pg. 105

Эти здоровенные луки традиционно изготавливаются из рогов крупных зверей, хотя современные жители Хонгала, самого северного государства Тянь Ся, для основы часто используют композитные материалы или даже небольшие деревья.
Хотя дальность стрельбы хонгальских роговых луков меньше, чем у других луков, они компенсируют это тем, что концентрируют мощь длинного лука на короткой дистанции и могут использоваться с лошади, что является идеальным вариантом для стремительного боевого стиля конных хонгальских войск.
Хотя жителям Авистана трудно заполучить это хонгальское оружие, небольшой отряд орков-наездников из Холда Белкзен сумел вооружиться хонгальскими роговыми луками и в течение нескольких лет оставлял кровавую полосу из авантюристов и соседних военных отрядов, пока не был окончательно разбит в отчаянной засаде.
Рассказы об этом отряде распространились, заставив авистанских искателей приключений, мало знакомых с Тянь Ся, ассоциировать эти луки с орками.

.. _weapon--Piranha-Kiss:

Поцелую пираньи (`Piranha Kiss <https://2e.aonprd.com/Weapons.aspx?ID=239>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Доступ**: Член :ref:`Смутьянов <Faction--Firebrands>`

**Источник**: Lost Omens: Grand Bazaar pg. 106

Изготовленное из зазубренного клинка с зубцами, направленными к обтянутой кожей рукояти, это оружие особенно эффективно для обезоруживания противников.
Поцелуи пираний иногда использовались во время Видрийской Революции, а некоторые :ref:`Смутьяны (Firebrands) <Faction--Firebrands>` носят их и сегодня.

.. _weapon--Sickle-Saber:

Серповидная сабля (`Sickle-Saber <https://2e.aonprd.com/Weapons.aspx?ID=240>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Доступ**: Персонажи из Иррисена

**Источник**: Lost Omens: Grand Bazaar pg. 106

Классическое иррисенское оружие, не обладавших магическим талантом, серповидная сабля видала возрождение после коронации королевы Анастасии.
Королева была очарована серповидными саблями из дворцовой сокровищницы, и теперь эти необычные клинки носят как ее почетный караул, так и дипломаты Иррисена.
Лезвие серповидной сабли неоднократно изгибается по всей длине 4-футовой длине, и ее рукоять имеет такой же изгиб.
Небольшой дополнительный хват на лезвии позволяет владельцу быстро и непредсказуемо крутить режущие края.

.. _weapon--Spiral-Rapier:

Спиральная рапира (`Spiral Rapier <https://2e.aonprd.com/Weapons.aspx?ID=241>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Доступ**: Персонажи из Талдора

**Источник**: Lost Omens: Grand Bazaar pg. 106

Старинное талданское дуэльное оружие времен расцвета империи, эта рапира имеет более толстый клинок по сравнению с обычным, который имеет форму штопорообразной спирали, хорошо подходящей для ловли оружия противника.

.. _weapon--Switchscythe:

Изменяемая коса (`Switchscythe <https://2e.aonprd.com/Weapons.aspx?ID=242>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: Lost Omens: Grand Bazaar pg. 106

Еще одно комплексное изобретение гномов, САБЖ отличается универсальностью.
Изогнутое лезвие частично полое, в нем находится длинный стержень из дерева или металла;
стержень можно вытянуть перпендикулярно лезвию, превращая косу из размашистого, похожего на топор клинка в крючковатую кирку, способную хватать противника.

.. _weapon--Thundermace:

Гремящая булава (`Thundermace <https://2e.aonprd.com/Weapons.aspx?ID=244>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: Lost Omens: Grand Bazaar pg. 106

Это обманчиво опасное оружие представляет собой, по сути, булаву с более длинным древком и крупным, зачастую фланцевым навершием.

.. _weapon--Throwing-Knife:

Метательный нож (Throwing Knife `LOGB <https://2e.aonprd.com/Weapons.aspx?ID=243>`_ / `AP#151 <https://2e.aonprd.com/Weapons.aspx?ID=113>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: Lost Omens: Grand Bazaar pg. 106,
Pathfinder #151: The Show Must Go On pg. 77

В отличие от обычного кинжала, этот легкий нож оптимально сбалансирован, чтобы точно метаться на большее расстояние.
Хотя это достигается за счет значительного уменьшенного режущей кромки, для специалиста по метательному оружию разница того стоит.

.. versionchanged:: /translation
	Метательный нож был первоначально добавлен в AP151.
	В LOGB он перестал быть :r_uncommon:`необычным` и у него убран признак :w_twin:`парное`.

.. _weapon--Tonfa:

Тонфа (`Tonfa <https://2e.aonprd.com/Weapons.aspx?ID=245>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: Lost Omens: Grand Bazaar pg. 106

Эти L-образные боевые дубинки хороши для ударов и блокирования.
Владелец держит рукоятку и либо вращает палку, либо наносит ей удар, закрывая предплечье.

.. _weapon--Tri-Bladed-Katar:

Трехлезвийный катар (`Tri-Bladed Katar <https://2e.aonprd.com/Weapons.aspx?ID=246>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: Lost Omens: Grand Bazaar pg. 106

Этот тычковый кинжал похож на стандартный :ref:`weapon--Tri-Bladed-Katar`, за исключением того, что от центрального лезвия могут выкидываться пара клинков, превращая оружие в звездообразную форму, хорошо подходящую для ловли оружия противника.








.. Character Guide

.. _weapon--Fauchard:

Фальшард (`Fauchard <https://2e.aonprd.com/Weapons.aspx?ID=90>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: Lost Omens: Character Guide pg. 92

Фальшард похож на глефу, за исключением того, что его режущий край расположен вдоль вогнутой стороны.
"Багровые восстановители" предпочитают фальшарды чтобы прорываться через волны зомби.
Фальшард является воинским оружием ближнего боя.

.. _weapon--Scourge:

Плеть (`Scourge <https://2e.aonprd.com/Weapons.aspx?ID=89>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: Lost Omens: Character Guide pg. 80

Плеть, так же известный как плетка девятихвостка - представляет собой набор из нескольких узловатых шнуров, сделанных из хлопка или кожи и прикрепленных к рукоятке.
Хотя большинство плетей больше подходят для пыток, чем для боя, оружие, используемое "Орденом Плети", имеет вплетенные металлические зубцы, чтобы пробивать одежду и доспехи.
Плеть - это воинское оружие ближнего боя.










.. World Guide

.. _weapon--Aldori-Dueling-Sword:

Дуэльный меч Алдори (`Aldori Dueling Sword <https://2e.aonprd.com/Weapons.aspx?ID=88>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: Lost Omens: World Guide pg. 28

Это тонкий однолезвенный дуэльный меч с небольшим изгибом клинка и острым укрепленным острием.

.. versionchanged:: /errata-LOWG-2
	Цена должна быть 2 зм, а не 20 зм.










.. Gods & Magic

.. _weapon--Bladed-Scarf:

Шарф с клинками (`Bladed Scarf <https://2e.aonprd.com/Weapons.aspx?ID=91>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: Lost Omens: Gods & Magic pg. 120

Тонкие металлические пластины, вплетенные в этот длинный шарф, превращают модный аксессуар в смертоносное оружие.

.. _weapon--Fighting-Fan:

Боевой веер (`Fighting Fan <https://2e.aonprd.com/Weapons.aspx?ID=93>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: Lost Omens: Gods & Magic pg. 120

Этот веер пригодится для элегантных танцев, а также для нарезания ничего не подозревающих врагов, лезвиями вдоль внешнего края.
Если он используется в выступлениях, то может быть замаскирован под вычурный аксессуар или это может быть очевидное, хоть и элегантное оружие.

.. _weapon--Khopesh:

Хопеш (`Khopesh <https://2e.aonprd.com/Weapons.aspx?ID=94>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: Lost Omens: Gods & Magic pg. 120

Этот изогнутый серповидный меч имеет заостренный наконечник, что позволяет размахивать им как ручным топором, или бить как коротким мечом.
Наконечник хопеша обычно крючкообразный, чтобы его можно было использовать чтобы обезоружить оппонента или выбить его щит.

.. _weapon--Machete:

Мачете (`Machete <https://2e.aonprd.com/Weapons.aspx?ID=95>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: Lost Omens: Gods & Magic pg. 120

Этот меч средней длины имеет широкое лезвие и длинную рукоятку.
Хотя мачете обычно используется для рубки густых зарослей, его также можно использовать в качестве смертоносного оружия.

.. _weapon--Mambele:

Молния (`Mambele <https://2e.aonprd.com/Weapons.aspx?ID=96>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: Lost Omens: Gods & Magic pg. 121

Этот гибридный нож-топор, также известный как хунга-мунга или даниско, состоит из рукояти и лезвия, которое изгибается назад к владельцу.
Изгиб клинка таков, что после удара молнией, жертве наносится больше урона при извлечении оружия из тела.

.. _weapon--Meteor-Hammer:

Молот-метеор (`Meteor Hammer <https://2e.aonprd.com/Weapons.aspx?ID=97>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: Lost Omens: Gods & Magic pg. 121

Это оружие состоит из длинной цепи, на одном конце соединенной с тяжелым грузом.
Когда владелец за цепь размахивается грузом, он набирает импульс и может служить смертельным дробящим оружием с невероятной досягаемостью.

.. _weapon--Naginata:

Нагината (`Naginata <https://2e.aonprd.com/Weapons.aspx?ID=98>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: Lost Omens: Gods & Magic pg. 121

Этот 6-футовый посох имеет, прикрепленный на одном конце, 2-футовый слегка изогнутый, похожий на меч клинок.
Длинный шест помогает держать владельца вне досягаемости мечей и более короткого оружия.

.. _weapon--Nine-Ring-Sword:

Меч девяти колец (`Nine-Ring Sword <https://2e.aonprd.com/Weapons.aspx?ID=99>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: Lost Omens: Gods & Magic pg. 121

Этот меч имеет широкий клинок, вдоль которого продеты 9 тяжелых металлических колец.
Кольца добавляют оружию вес для рубящих размахов, а также сталкиваются друг с другом, чтобы издавать шум.

.. _weapon--Polytool:

Мультитул (`Polytool <https://2e.aonprd.com/Weapons.aspx?ID=100>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: Lost Omens: Gods & Magic pg. 121

Это маленький металлический жезл с многочисленными простыми инструментами сложенными внутрь.
Пользователь может выдвинуть длинное керамическое лезвие, шило, зубило, напильник, огниво, крюк, чернильную ручку, увеличительное стекло, плоскогубцы, ножницы и маленькую пилу.
Огниво можно использовать 10 раз, после чего его надо заменить.
Несмотря на то, что мультитул вдохновлен передовыми Нумерскими технологиями, он достаточно прост для металлообработки, чтобы любой кузнец мог его изготовить - идеально подходит для богини Касандали, чтобы распространить инновации дальше, чем может достичь Нумерская технология.

.. _weapon--Tekko-kagi:

Сюко (`Tekko-kagi <https://2e.aonprd.com/Weapons.aspx?ID=101>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: Lost Omens: Gods & Magic pg. 121

Четыре изогнутых лезвия, прикрепленных к крепкой рукояти, дают владельцу этого оружия ближнего боя иллюзию наличия когтей, которые выходят от их кулака.
Приверженцы Бастет отдают предпочтение сюко за то, что они застают своих врагов врасплох.

.. _weapon--Urumi:

Уруми (`Urumi <https://2e.aonprd.com/Weapons.aspx?ID=102>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: Lost Omens: Gods & Magic pg. 121

От рукояти этого меча идет набор хлыстовидных лезвий.

.. _weapon--War-Razor:

Боевая бритва (`War Razor <https://2e.aonprd.com/Weapons.aspx?ID=103>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: Lost Omens: Gods & Magic pg. 121

Боевая бритва - увеличенная версия парикмахерского инструмента.
Это хрупкое, но чрезвычайно острое оружие, которое очень легко сунуть в карман или рукав.










.. Bestiary

.. _weapon--Ogre-Hook:

Крюк огра (`Ogre Hook <https://2e.aonprd.com/Weapons.aspx?ID=86>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: Bestiary pg. 253

Огры известны тем, что используют огромные изогнутые кирки, называемые крюками огров.

.. _weapon--Aklys:

Аклис (`Aklys <https://2e.aonprd.com/Weapons.aspx?ID=85>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: Bestiary pg. 85

Это метательная дубинка с крюком на одном конце и длинным шнуром, прикрепленным к другому.
Хотя аклисы не доступны в большинстве магазинов, один из них можно купить за 5 gp у поставщика, специализирующегося на странном оружии.










.. Web Supplement (WS)

.. _weapon--Boarding-Axe:

Абордажный топор (`Boarding Axe <https://2e.aonprd.com/Weapons.aspx?ID=155>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: Azarketi Ancestry Web Supplement pg. 1

Этот небольшой топор имеет шип с противоположной стороны от лезвия, который помогает в лазании и полезен при расчистке препятствий, таких как упавшие снасти.
Это оружие распространено в районе Открытого моря, на острове Кортос и в поселениях азаркетов.

.. _weapon--Gill-Hook:

Жаберный крюк (`Gill Hook <https://2e.aonprd.com/Weapons.aspx?ID=156>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: Azarketi Ancestry Web Supplement pg. 1

Это копье имеет специальный крючок непосредственно перед наконечником, которым можно зацепиться за жабры крупной рыбы.
Азаркеты в основном используют его для охоты на акул, но он также может быть использован, чтобы зацепить плоть или доспех.
Это оружие распространено в районе Открытого моря, на острове Кортос и в поселениях азаркетов.










.. Ancestry Guide (LOAG)

.. _weapon--Adze:

Тесло (`Adze <https://2e.aonprd.com/Weapons.aspx?ID=141>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: Lost Omens: Ancestry Guide pg. 138

Распространенный режущий инструмент, тесло напоминает топор, но режущая кромка расположена горизонтально, а не вертикально.
Форма тесла делает его популярным среди деревообработчиков, и гриппли-строители часто используют его для строительства своих домов на верхушках деревьев.
Инструмент также служит эффективным оружием, отчасти из-за огромной силы, необходимой, чтобы им замахиваться.

.. _weapon--Hand-Adze:

Ручное тесло (`Hand Adze <https://2e.aonprd.com/Weapons.aspx?ID=144>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: Lost Omens: Ancestry Guide pg. 138

Это уменьшенная версия тесла, пригодная для более деликатной обработки дерева или работы в тесном пространстве.
Гриппли используют ручное тесло для ремесла и в качестве оружия ближнего боя.

.. _weapon--Buugeng:

Бууген (`Buugeng <https://2e.aonprd.com/Weapons.aspx?ID=142>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: Lost Omens: Ancestry Guide pg. 138

Клинок конструкции конрасов, бууген имеет уникальную изогнутую форму, которая позволяет ему плавно вращаться в руке обученного воина.
Это вращательное движение облегчает атаку сразу нескольких противников с помощью оружия.

.. _weapon--Fangwire:

Струноклык (`Fangwire <https://2e.aonprd.com/Weapons.aspx?ID=143>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: Lost Omens: Ancestry Guide pg. 138

Эта кобольдская струна тонкая и едва заметная, что делает ее идеальной для засады.
Владелец обматывает струну вокруг уязвимого места, например, на шею, и скручивает ее, чтобы нанести потенциально смертельные порезы.
Название происходит от использования аналогичной проволоки в ловушках, или как они их называют "медленных клыках", с убранным словом "медленный", так как струноклык дает быстрые результаты.

.. _weapon--Rungu:

Рангу (`Rungu <https://2e.aonprd.com/Weapons.aspx?ID=145>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: Lost Omens: Ancestry Guide pg. 138

Эта специализированная дубинка предназначена для метания и полезна как для боя, так и для охоты.
Хотя гриппли изначально не создавал рангу, многие общины гриппли приняли его в качестве оружия для охоты на существ, которые прячутся высоко в кронах деревьев.

.. _weapon--Taw-Launcher:

Метатель тау (`Taw Launcher <https://2e.aonprd.com/Weapons.aspx?ID=146>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: Lost Omens: Ancestry Guide pg. 138

Разработанное воинами конрасу, это сложное устройство напоминает арбалет и стреляет маленькими деревянными пулями, известными как тау.
Система лезвий внутри метателя может быстро изменять форму тау по мере его зарядки, позволяя метателю стрелять тау различной формы, например флешеттами.

.. _weapon--Thunder-Sling:

Громовая праща (`Thunder Sling <https://2e.aonprd.com/Weapons.aspx?ID=147>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: Lost Omens: Ancestry Guide pg. 138

Тэнгу используют эти специальные пращи для метания дротиками дальше и с большей силой, чем при броске вручную, что особенно эффективно при атаке с больших высот.
В качестве амуниции громовая праща использует дротики.
Она также может метать дротики для духового ружья, но при таком использовании наносит 1d4 колющего урона вместо 1d6.

**Прим.пер.**: Это копьеметалка.

.. _weapon--Tricky-Pick:

Хитрая кирка (`Tricky Pick <https://2e.aonprd.com/Weapons.aspx?ID=148>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: Lost Omens: Ancestry Guide pg. 138

Эта хитроумная кобольдская кирка скрывает несколько припрятанных ловушек, которые владелец может активировать, чтобы обмануть и одурачить врагов различными наносящими урон лезвиями и бьющими поверхностями.

.. _weapon--Whip-Claw:

Хлыст-коготь (`Whip Claw <https://2e.aonprd.com/Weapons.aspx?ID=149>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: Lost Omens: Ancestry Guide pg. 138

Хлыст-коготь это длинный трос, прикрепленный к когтистым кинжалам, что позволяет владельцу метать и притягивать их со смертельной точностью.
Котолюди разработал это оружие, чтобы обеспечить увеличенную дальность атаки при охоте на опасных животных и владеют им с непревзойденным мастерством.

.. _weapon--Wish-Blade:

Меч желания (`Wish Blade <https://2e.aonprd.com/Weapons.aspx?ID=150>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: Lost Omens: Ancestry Guide pg. 138

Специализированные насечки, выложенные уникальными, алхимически обработанными металлами, способны удерживать энергию, по всей длине этого меча.
Первые мечи желаний были созданы кузнецами-гениями, и знания об этом оружии передавались поколениям джинникинов, благодаря чему они получили название мечи желаний.

.. _weapon--Wish-Knife:

Нож желания (`Wish Knife <https://2e.aonprd.com/Weapons.aspx?ID=151>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: Lost Omens: Ancestry Guide pg. 138

Подобно мечу желания, этот нож по всей длине исчерчен замысловатыми насечками, способными удерживать энергию.
Ножи желания легче своих собратьев, что делает их предпочтительным оружием для ловких бойцов.



.. The Mwangi Expanse (LOME)

.. _weapon--Fighting-Stick:

Боевая палка (`Fighting Stick <https://2e.aonprd.com/Weapons.aspx?ID=168>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: The Mwangi Expanse pg. 86

**Доступ**: этнос Сон'о

Этот твердый, но гибкий кусок древесины, длиной с длинный меч, больше похож на шест, чем на оружие, но в умелых руках может быть смертельно опасным.
Хотя обычно они не используются для боя за пределами культуры Сон'о, некоторые полурослики стали настолько опытными, что могут использовать его в бою так же эффективно, как и клинок.
Многие полурослики даже будут петь, чтобы поддерживать во время боя определенный темп и ритм.


.. _weapon--Gaff:

Багор (`Gaff <https://2e.aonprd.com/Weapons.aspx?ID=169>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: The Mwangi Expanse pg. 256, Bestiary 2 pg. 125

Эти крючковатые посохи популярны как среди Сенгорских рыболовов, так и среди воинов.
Багры доступны в Сенгоре и близлежащих поселениях по цене в 1 зм.





.. Adventure Path

.. _weapon--Juggling-Club:

Булава для жонглирования (`Juggling Club <https://2e.aonprd.com/Weapons.aspx?ID=111>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: Pathfinder #151: The Show Must Go On pg. 77

Жонглерская булава легче обычной и сбалансирована так, что жонглер ее легко поймает и снова бросит.
В то время, как жонглерская дубинка наносит меньше урона, дополнительная дистанция броска, которую позволяет ее легкий вес, важна для жонглирования (TODO ссылка способность).

.. _weapon--Poi:

Пои (`Poi <https://2e.aonprd.com/Weapons.aspx?ID=112>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: Pathfinder #151: The Show Must Go On pg. 77

Пои - это легкие веса, привязанные к веревкам или цепям.
Исполнители размахивают этими грузами, обычно по одному в каждой руке, в ритмической форме.

.. _weapon--Fire-Poi:

Горящий пои (`Fire Poi <https://2e.aonprd.com/Weapons.aspx?ID=117>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: Pathfinder #151: The Show Must Go On pg. 77

Эти специальные пои сделаны из редкого, легкого металла или из огнестойких волокон и могут быть зажжены перед использованием.
Зажигание пары огненных пои является действием :ref:`action--Interact` и требует 1 пинты масла на каждые 10 минут, когда пои остается зажженным.
Пока горят, пои отбрасывают тусклый свет в радиусе 10 футов; в бою, они наносят 1d4 дробящего урона и 1d4 огненного урона.
При крит.попадании горящим пои, цель получает 1 продолжительного урона огнем.
Огонь можно потушить с помощью действия :ref:`action--Interact`.
Когда не зажжены, пои наносят указанный дробящий урон.
Независимо от того, зажжены ли пои, дробящий урон пои 1d4 - кости урона оружия, поэтому *разящие руны* и другие эффекты не влияют на урон от огня.

.. _weapon--Bladed-Diabolo:

Диаболо с лезвиями (`Bladed Diabolo <https://2e.aonprd.com/Weapons.aspx?ID=115>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: Pathfinder #151: The Show Must Go On pg. 77

Это оружие состоит из двух лезвий-дисков, соединенных центральной осью, и вращающихся на веревке, концы которой прикреплены к палочкам-жезлам.
Владелец может швырнуть диаболо с веревки, как камень с пращи, или размахивать им на веревке в ближнем бою.

.. _weapon--Bladed-Hoop:

Обруч с лезвиями (`Bladed Hoop <https://2e.aonprd.com/Weapons.aspx?ID=116>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: Pathfinder #151: The Show Must Go On pg. 77

Этот круглый обруч имеет лезвия вдоль внешнего края.
Вы можете владеть им двумя руками (используя двуручный урон) или вращать его вокруг руки.
Когда вы вращаете его, он получает признак "свободная рука".
Заставить обруч вращаться занимает одно действие :ref:`action--Interact`.
В дополнение к обычным ограничениям признака "свободная рука", расширенное использование руки для передвижения (такое как :ref:`skill--Athletics--Climb`) мешает вращению обруча и заставляет вас :ref:`action--Release` его.
Вы не можете вращать обруч под водой.
Поддерживать вращение обруча, требует каждый раунд использования свободного действия (|д-св|) с признаками "концентрация" и "воздействие".
Вы можете вращать обруч в качестве активности режима исследования, но делая так больше 10 минут сделает вас "утомленным", как при использовании :ref:`expl-activity--Hustle`

.. _weapon--Scorpion-Whip:

Хлыст скорпиона (`Scorpion Whip <https://2e.aonprd.com/Weapons.aspx?ID=114>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: Pathfinder #151: The Show Must Go On pg. 77

Хлыст скорпиона имеет ряд острых как бритва лезвий, установленных вдоль его кончика.
В отличие от обычных кнутов, у хлыста скорпиона отсутствует признак "несмертельное", что делает его более смертоносным в бою, но менее эффективным, когда владелец хочет привести врага живым.

.. _weapon--Battle-Lute:

Боевая лютня (`Battle Lute <https://2e.aonprd.com/Weapons.aspx?ID=153>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: Pathfinder #164: Hands of the Devil pg. 73

Эта усиленная лютня подходит как для использования в качестве ручного музыкального инструмента, так и для битья голов, если толпа станет несносной.
Его струны - тонко сплетенные проволоки, которые проходят вдоль его прочного металлического грифа.

.. _weapon--Tamchal-Chakram:

Тамчал Чакрам (`Tamchal Chakram <https://2e.aonprd.com/Weapons.aspx?ID=154>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: Pathfinder #164: Hands of the Devil pg. 73

Это круглое оружие входит в число многих странных видов оружия, используемых урдефанами.
Острый металлический круг содержит множество выступающих лезвий, в то время как центральная рукоятка под углом обеспечивает крепкий захват, закручивающий оружие при броске.

.. _weapon--Rhoka-Sword:

Меч рока (`Rhoka Sword <https://2e.aonprd.com/Weapons.aspx?ID=158>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: Pathfinder #165: Eyes of Empty Death pg. 73

Эти двухлезвийные мечи используются воинами урдефанами.

.. _weapon--Shauth-Blade:

Клинок шаут (`Shauth Blade <https://2e.aonprd.com/Weapons.aspx?ID=157>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: Pathfinder #165: Eyes of Empty Death pg. 73

Эти странные изогнутые клинки представляют собой зазубренное и смертоносное оружие, сделанное из алхимически укрепленных зубов мертвых урдефанов.
Каждое оружие обычно называют в честь урдефана, зубы которого были выкованы в оружие, которое часто носят (и почитают) потомки этого урдефана.
Магические клинки шаут позволяют владельцу урдефану направлять его способность Злой укус (Wicked Bite) с помощью ударов клинка.

.. _weapon--Shauth-Lash:

Плеть шаут (`Shauth Lash <https://2e.aonprd.com/Weapons.aspx?ID=159>`_)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Источник**: Pathfinder #165: Eyes of Empty Death pg. 73

Эта металлическая цепь украшена подобными крюкам шипами из алхимически укрепленных зубов мертвых ундерфанов.
Магические плети шаут также позволяют владельцу урдефану направлять его способность Злой укус (Wicked Bite), и урдефаны держат это оружие с таким же почтением, как и клинки шаут.











.. include:: /helpers/actions.rst