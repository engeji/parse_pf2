...
| **Провал**: Цель отталкивается на 10 футов от вас
| **Критический провал**: Цель отталкивается на 10 футов от вас и падает на землю распластавшись
...
| **Критический провал**: Как успех, но продолжительность 1 минута, и цель так же получает состояние "слепота" на время продолжительности. Она может сделать новый спасбросок в конце каждого своего хода. В случае успеха она перестает быть слепой, но ночное или сумеречное зрение остается подавленным.
...
| **Провал**: Существо получает полный урон и падает на землю распластавшись
| **Критический провал**: Существо получает двойной урон и падает на землю распластавшись
...
**Триггер**: Вы и 1 или более союзников в пределах дистанции стали целью заклинания или способности которая позволяет совершить спасбросок
...
Если провал, то становится крит.провалом, и этот крит.провал не может быть улучшен с помощью способностей, который обычно улучшают степень успешности крит.провала, как :ref:`class-feature--ranger--Improved-Evasion`.
...
Цель восстанавливает дополнительные ОЗ от *благословения целителя* только первый раз восстанавливая ОЗ от полученного заклинания, поэтому заклинание, которое исцеляет несколько раз в течение времени, восстанавливает дополнительные ОЗ только один раз.
...
Она получает еды достаточного для целого приема пиши и должно совершить спасбросок Стойкости.
...
| **Провал**: Как успех, но вместо вас, "замешательство" получает цель, тратя на это свое первое действие каждый ход. Продолжительность 1 минута
...
Когда существо входит в область в первый раз, оно должно совершить спасбросок Воли.
...
| **Критический провал**: Существо "заворожено" вами, и ее отношение к вам улучшается на 1 ступень
...
| **Провал**: Существо получает состояния "слепота" на 1 раунд и "ослеплен" на 1 минуту. Существо может потратить действие для :ref:`action--Interact` чтобы закончить состояние "слепой".
...
| **Провал**: Как успех, но цель должна успешно пройти спасбросок Воли, чтобы не поверить в иллюзию, когда случается одно из описанных событий
| **Критический провал**: Как успех, но цель должна крит.успешно пройти спасбросок Воли чтобы не поверить в иллюзию, когда случается одно из описанных событий
...
| **Провал**: Цель получает полный урон и позитивные эффекты исцеления восстанавливают ей только половину ОЗ на 1 раунд
| **Критический провал**: Цель получает двойной урон и позитивные эффекты исцеления восстанавливают ей только половину ОЗ на 1 раунд
...
Каждое существо в пределах 20 футов от области, которое может польститься на материальное богатство, должна совершить спасбросок Воли.
...
Целевой объект превращается из обычного в холодное железо, медь, золото, железо, серебро или сталь (см. подробности в :ref:`ch11--Materials`).
...
**Усиление (8-й)**: Добавьте :ref:`material--Adamantine` и :ref:`material--Mithral` к списку металлов, в которые вы можете превращать предмет.
