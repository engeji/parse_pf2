.. include:: /helpers/roles.rst

.. rst-class:: spell
.. _spell--c--Command:

Приказ (`Command <https://2e.aonprd.com/Spells.aspx?ID=45>`_) / Закл. 1
"""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""

- очарование
- слуховой
- языковой
- ментальный

**Обычай**: арканный, сакральный, оккультный

**Сотворение**: |д-2| жестовый, словесный

**Дистанция**: 30 футов

**Цели**: 1 существо

**Спасбросок**: Воля

**Продолжительность**: до конца следующего хода цели

----------

Вы выкрикиваете команду, которую трудно проигнорировать.
Вы можете приказать цели приблизиться к вам, убежать (как если бы у нее было состояние :c_fleeing:`бегство`), отпустить то что она держит, упасть ничком, или стоять на месте.
Она не может :ref:`action--Delay` или использовать никакие реакции, пока не выполнит ваш приказ.
Эффекты зависят от спасброска Воли.

| **Успех**: Существо невредимо
| **Провал**: В качестве первого действия во время следующего хода, существо обязано использовать одно действие, чтобы выполнить ваш приказ
| **Критический провал**: Существо обязано использовать все свои действия во время следующего хода, чтобы подчиниться вашему приказу

----------

**Усиление (5-й)**: Вы можете выбрать целями вплоть до 10 существ.





.. include:: /helpers/actions.rst