.. include:: /helpers/roles.rst

.. rst-class:: spell
.. _spell--b--Blister:

Волдырь (`Blister <https://2e.aonprd.com/Spells.aspx?ID=673>`_) / Закл. 5
""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""

- некромантия

**Обычай**: арканный, оккультный, природный

**Сотворение**: |д-2| жестовый, словесный

**Дистанция**: 60 футов

**Цели**: 1 существо

**Спасбросок**: Стойкость

**Продолжительность**: 1 минута

**Источник**: Advanced Player's Guide pg. 216

----------

Вы указываете на цель и на ее коже появляются жгучие волдыри, наполненные едкой жидкостью.
Цель должна совершить спасбросок Стойкости.

| **Критический успех**: Цель невредима
| **Успех**: На цели вырастает 1 волдырь. Вы можете использовать одно действие, которое имеет признак :t_concentrate:`концентрация`, чтобы лопнуть волдырь. Цель и каждое существо в 15-футовом конусе исходящем от цели, получают 7d6 урона кислотой (простой спасбросок Стойкости). Вы выбираете направление конуса, которое не может включать пространство самой цели. Когда волдырей не остается, заклинание завершается.
| **Провал**: Как успех, но на цели вырастает 2 волдыря
| **Критический провал**: Как успех, но на цели вырастает 4 волдыря

----------

**Усиление (+1)**: Урон от лопнувшего волдыря увеличивается на 1d6.





.. include:: /helpers/actions.rst